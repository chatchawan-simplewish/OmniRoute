# OmniRoute V24 dashboard-home route-shape diagnostic brief
...
failed in the consumed V23 prestart gate. Its direct predecessor is incident
...
V24 does not retry or continue V23. It creates one new owned tab, navigates once
...
```javascript
let secureConsoleDashboardV24DiagnosticConsumed = false;
let secureConsoleDashboardV24RetainedTab = null;
let secureConsoleDashboardV24State = "UNCREATED";
await (async () => {
  const gateWasFresh = secureConsoleDashboardV24DiagnosticConsumed === false;
  secureConsoleDashboardV24DiagnosticConsumed = true;
  const counters = {
  ...
  try {
    declarationShape =
      ...
      typeof secureConsoleDashboardV24DiagnosticConsumed === "boolean";
    predecessorExact =
      declarationShape &&
      gateWasFresh === true &&
      ...
      secureConsoleDashboardV24RetainedTab === null &&
      secureConsoleDashboardV24State === "UNCREATED";
    if (!predecessorExact) throw new Error("DashboardDiagnosticPreconditionError");

    counters.newAttempted++;
    secureConsoleDashboardV24RetainedTab = tab =
    ...
    controllerOwnership = secureConsoleDashboardV24RetainedTab === tab;
    tabShape =
      controllerOwnership &&
    ...
    secureConsoleDashboardV24State = "DASHBOARD_DIAGNOSTIC_RUNNING_V24";
    ...
    result = "EXACT_V24_DASHBOARD_ROUTE_SHAPE_DIAGNOSTIC_PASS";
  } catch (error) {
  ...
  if (tab !== null && tab !== undefined && typeof tab.close === "function") {
    ...
    try {
      ...
      secureConsoleDashboardV24RetainedTab = null;
      secureConsoleDashboardV24State =
        result === "EXACT_V24_DASHBOARD_ROUTE_SHAPE_DIAGNOSTIC_PASS"
          ? "V24_DIAGNOSTIC_PASS_CLEAN"
          : "V24_DIAGNOSTIC_FAILED_CLEAN";
      cleanupState = "EXACT_TAB_CLOSED";
      residueConverged = true;
    } catch (closeError) {
      if (errorClass === "NONE") errorClass = safeErrorClass(closeError);
      secureConsoleDashboardV24RetainedTab = tab;
      secureConsoleDashboardV24State = "V24_DIAGNOSTIC_FAILED_TAB_RETAINED";
      cleanupState = "EXACT_TAB_CLOSE_REJECTED";
      residueConverged = false;
      result = "PRECONDITION_FAIL";
    }
  } else if (counters.newAttempted === 0) {
    secureConsoleDashboardV24State = "V24_DIAGNOSTIC_FAILED_CLEAN";
    cleanupState = "NO_TAB_CREATED";
  } else {
    secureConsoleDashboardV24State = "V24_DIAGNOSTIC_FAILED_RESIDUE_UNPROVEN";
  ...
  nodeRepl.write({
    ...
    cleanupState,
    residueConverged,
    retainedExactHandle: secureConsoleDashboardV24RetainedTab !== null,
    state: secureConsoleDashboardV24State,
    consumed: secureConsoleDashboardV24DiagnosticConsumed,
    ...counters,
    errorClass,
  });
})();
...
`V24_DIAGNOSTIC_PASS_CLEAN`; residue converged; consumed true; error
...
Any non-PASS spends V24. No retry, fallback, continuation, reinterpretation,
...
one-shot state, exact new-handle ownership, fixed target, wait/evaluator safety,
