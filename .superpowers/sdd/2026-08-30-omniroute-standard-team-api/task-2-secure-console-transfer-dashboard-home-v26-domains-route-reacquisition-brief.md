# OmniRoute V26 guarded Domains-route reacquisition replacement brief
...
```javascript
let secureConsoleDashboardV26RouteConsumed = false;
let secureConsoleDashboardV26RetainedTab = null;
let secureConsoleDashboardV26Eligible = false;
let secureConsoleDashboardV26State = "UNCREATED";
let secureConsoleDashboardV26PreCreateDetachConsumed = false;
let secureConsoleDashboardV26PostNativeDetachConsumed = false;
let secureConsoleCloudflareReadsV26Consumed = false;
await (async () => {
  const gateWasFresh = secureConsoleDashboardV26RouteConsumed === false;
  ...
  try {
    ...
    predecessorExact =
      ...
      secureConsoleDashboardV26State === "UNCREATED" &&
    ...
    secureConsoleDashboardV26RetainedTab = tab =
    ...
    controllerOwnership = secureConsoleDashboardV26RetainedTab === tab;
    tabShape =
      controllerOwnership &&
    ...
    secureConsoleDashboardV26State = "DOMAINS_ROUTE_RUNNING_V26";
    ...
    secureConsoleDashboardV26Eligible = true;
    secureConsoleDashboardV26State = "ZONE_ROUTE_READY_ELIGIBLE_V26";
    cleanupState = "SUCCESS_TAB_RETAINED";
    residueConverged = true;
    result = "EXACT_V26_DOMAINS_ROUTE_REACQUISITION_PASS";
  } catch (error) {
    errorClass = safeErrorClass(error);
  }

  if (result !== "EXACT_V26_DOMAINS_ROUTE_REACQUISITION_PASS") {
    secureConsoleDashboardV26Eligible = false;
    if (tab !== null && tab !== undefined && typeof tab.close === "function") {
      ...
      try {
        ...
        secureConsoleDashboardV26RetainedTab = tab;
        secureConsoleDashboardV26State = "V26_ROUTE_FAILED_TAB_RETAINED";
      ...
      secureConsoleDashboardV26State = "V26_ROUTE_FAILED_RESIDUE_UNPROVEN";
  ...
  nodeRepl.write({
    ...
    cleanupState,
    residueConverged,
    retainedExactHandle: secureConsoleDashboardV26RetainedTab !== null,
    eligible: secureConsoleDashboardV26Eligible,
    state: secureConsoleDashboardV26State,
    routeConsumed: secureConsoleDashboardV26RouteConsumed,
    readsConsumed: secureConsoleCloudflareReadsV26Consumed,
    preCreateDetachConsumed: secureConsoleDashboardV26PreCreateDetachConsumed,
    postNativeDetachConsumed: secureConsoleDashboardV26PostNativeDetachConsumed,
    ...counters,
    errorClass,
  });
})();
...
both detach flags false; state `ZONE_ROUTE_READY_ELIGIBLE_V26`; error `NONE`;
and completed tool output.

Any non-PASS spends V26. No retry, fallback, continuation, reinterpretation,
...
evidence worktree, zero residue, VM1205/DNS safe state, exact V23/V25 persistent
state, V24 declaration absence, and every V26 declaration absent.
