# OmniRoute V29 Domains-button Enter expansion diagnostic brief

`authorizes_live_execution=false`

## Design and scope

V28 consumed once and passed clean at evidence commit
`61f5233ee997ed5ea9f0e6b0561a375453301e94`. It proved the unique visible
Domains control is an enabled, hit-testable, keyboard-reachable native button
with an existing `aria-controls` target. It is not an anchor, exposes no href,
and exposes no element-level native `click()` method.
...
V29 opens one fresh owned tab through the persistent V5 controller, restores
...
failure, or consumed state spends V29 permanently; there is no retry,
...
```javascript
let secureConsoleDashboardV29ExpansionConsumed = false;
let secureConsoleDashboardV29RetainedTab = null;
let secureConsoleDashboardV29State = "UNCREATED";
await (async () => {
  const gateWasFresh = secureConsoleDashboardV29ExpansionConsumed === false;
  ...
  const preIntegerKeys = ["buttonCount", "visibleButtonCount", "busyCount"];
  ...
  try {
    ...
    controllerOwnership = secureConsoleDashboardV29RetainedTab === tab;
    tabShape =
      controllerOwnership &&
    ...
    secureConsoleDashboardV29State = "DOMAINS_EXPANSION_RUNNING_V29";
    ...
    preShapeExact =
      ...
      preSnapshot.controlsTargetVisible === false &&
      ...
      async (accountSegment) => {
        ...
        while (true) {
          ...
          const accountHomePath =
            new RegExp(`^/${accountSegment}/home/?$`).test(location.pathname);
          const accountDomainsPath =
            new RegExp(`^/${accountSegment}/home/domains/?$`).test(
              location.pathname,
            );
          ...
          const settleConditionMet = accountDomainsPath || expandedTrue ||
            targetVisible || controlledAnchors.length > 0;
    ...
    result = "EXACT_V29_DOMAINS_BUTTON_ENTER_DIAGNOSTIC_PASS";
  } catch (error) {
  ...
  if (tab !== null && tab !== undefined && typeof tab.close === "function") {
    ...
    try {
      ...
      secureConsoleDashboardV29RetainedTab = null;
      secureConsoleDashboardV29State =
        result === "EXACT_V29_DOMAINS_BUTTON_ENTER_DIAGNOSTIC_PASS"
          ? "V29_DIAGNOSTIC_PASS_CLEAN"
          : "V29_DIAGNOSTIC_FAILED_CLEAN";
      cleanupState = "EXACT_TAB_CLOSED";
      residueConverged = true;
    } catch (closeError) {
      if (errorClass === "NONE") errorClass = safeErrorClass(closeError);
      secureConsoleDashboardV29RetainedTab = tab;
      secureConsoleDashboardV29State = "V29_DIAGNOSTIC_FAILED_TAB_RETAINED";
    ...
    secureConsoleDashboardV29State = "V29_DIAGNOSTIC_FAILED_RESIDUE_UNPROVEN";
  ...
  nodeRepl.write({
    ...
    cleanupState,
    residueConverged,
    retainedExactHandle: secureConsoleDashboardV29RetainedTab !== null,
    state: secureConsoleDashboardV29State,
    consumed: secureConsoleDashboardV29ExpansionConsumed,
    ...counters,
    errorClass,
  });
})();
...
- revalidate the exact source projection, empty index, exact 12-path dirty
  ...
  predecessor state, and absent V29 declarations;
...
receives the executable consumes V29 before any precondition is evaluated.
...
no retained exact handle, state `V29_DIAGNOSTIC_PASS_CLEAN`, consumed true,
...
created handle when possible, record sanitized fixed evidence, classify V29
spent, and stop. Never retry or continue V29.
