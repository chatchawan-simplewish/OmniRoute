# Task 2 secure-console token-page readiness V11 structural diagnostic brief

Date: 2026-09-02 (Asia/Bangkok)

## Purpose and boundary

V10 is consumed and failed cleanly before evaluation because the exact
placeholder input was not yet present immediately after navigation. Its
immutable incident is commit `8e5113987`. V11 never retries or continues V10.
It is the smallest replacement: one bounded visibility wait for the same
placeholder, followed by the unchanged read-only structural diagnostic for
safe association of that input with one of the two tables and the current
non-mutating Create-control shape.

V11 creates one controller-owned tab, navigates only to the fixed public token
URL, waits at most 10000 ms for the exact placeholder locator to become visible,
evaluates one bounded count/boolean diagnostic, and closes that exact tab.
...
diagnostic fulfillment or body completion. V11 PASS authorizes only offline
interpretation and a new reviewed readiness contract.

## One-shot and no-residue contract

V11 contains exactly one `tabs.new()`, one fixed `goto()`, one bounded
...
failure, timeout, truncation, tool error, or uncertainty consumes V11 and
...
Executable bytes: `18310`.

Executable SHA-256:
`F1A15636FB741F7B452727E929932E3A4B91FD42FD247AFE0738AFBCE8E95785`.

```javascript
let secureConsoleV11StructuralDiagnosticConsumed = false;
let secureConsoleV11RetainedTab = null;
await (async () => {
  const gateWasFresh = secureConsoleV11StructuralDiagnosticConsumed === false;
  ...
  try {
    ...
    secureConsoleV11RetainedTab = tab = await secureConsoleChromeV5.tabs.new();
    ...
    result = "V11_STRUCTURAL_DIAGNOSTIC_BODY_COMPLETE";
  } catch (error) {
  ...
  if (
    ...
    result = "EXACT_V11_STRUCTURAL_DIAGNOSTIC_PASS";
  }
  ...
  nodeRepl.write({
    ...
    retainedExactHandle: secureConsoleV11RetainedTab !== null,
    ...counters,
    errorClass,
    consumed: secureConsoleV11StructuralDiagnosticConsumed,
...
Exact success is `EXACT_V11_STRUCTURAL_DIAGNOSTIC_PASS` with completed new,
navigation, URL, wait, count, diagnostic, close, and write counters; fixed target
true; exact close; converged residue; null retained handle; and unchanged
null/ineligible V4 state.

## Preserved constraints

V11 PASS authorizes only offline interpretation and a fresh independently
reviewed readiness replacement. The bounded wait changes readiness timing only;
the evaluator, safe-output validation, and no-mutation boundary remain unchanged. V4 prestart and provider mutation remain
