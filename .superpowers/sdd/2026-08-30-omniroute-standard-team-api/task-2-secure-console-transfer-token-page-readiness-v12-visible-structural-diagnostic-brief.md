# Task 2 secure-console token-page readiness V12 structural diagnostic brief

Date: 2026-09-02 (Asia/Bangkok)

## Purpose and boundary

V11 is consumed and failed cleanly before evaluation because the ready page
contained two inputs matching the unfiltered placeholder predicate. Its
immutable incident is commit `612aff1c2`. V12 never retries or continues V11.
It is the smallest replacement: constrain the same placeholder predicate to
visible inputs, require exactly one visible match, then run the unchanged
read-only structural diagnostic for safe association of that input with one of
the two tables and the current non-mutating Create-control shape.

V12 creates one controller-owned tab, navigates only to the fixed public token
URL, waits at most 10000 ms for the visible placeholder locator, evaluates one
bounded count/boolean diagnostic, and closes that exact tab.
...
diagnostic fulfillment or body completion. V12 PASS authorizes only offline
interpretation and a new reviewed readiness contract.

## One-shot and no-residue contract

V12 contains exactly one `tabs.new()`, one fixed `goto()`, one bounded
...
failure, timeout, truncation, tool error, or uncertainty consumes V12 and
...
Executable bytes: `18502`.

Executable SHA-256:
`18C66B1B29B1A547EC381211669589FE8F9BBA2F3682C9F7F50F51BD0A877BA4`.

```javascript
let secureConsoleV12StructuralDiagnosticConsumed = false;
let secureConsoleV12RetainedTab = null;
await (async () => {
  const gateWasFresh = secureConsoleV12StructuralDiagnosticConsumed === false;
  ...
  try {
    ...
    secureConsoleV12RetainedTab = tab = await secureConsoleChromeV5.tabs.new();
    ...
    result = "V12_STRUCTURAL_DIAGNOSTIC_BODY_COMPLETE";
  } catch (error) {
  ...
  if (
    ...
    result = "EXACT_V12_STRUCTURAL_DIAGNOSTIC_PASS";
  }
  ...
  nodeRepl.write({
    ...
    retainedExactHandle: secureConsoleV12RetainedTab !== null,
    ...counters,
    errorClass,
    consumed: secureConsoleV12StructuralDiagnosticConsumed,
...
Exact success is `EXACT_V12_STRUCTURAL_DIAGNOSTIC_PASS` with completed new,
...
V12 PASS authorizes only offline interpretation and a fresh independently
reviewed readiness replacement. The visible predicate narrows only candidate
selection; the evaluator, safe-output validation, and no-mutation boundary
remain unchanged. V4 prestart and provider mutation remain
