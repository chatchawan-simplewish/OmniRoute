# Task 2 secure-console token-page readiness V14 null-prototype structural diagnostic brief

Date: 2026-09-02 (Asia/Bangkok)

## Purpose and boundary

V13 is consumed and isolated the sole validation failure to local
`Object.prototype` identity. Its immutable incident is commit `e95bdc321`.
V14 never retries or continues V13. It retains the same visible selector,
byte-identical page evaluator, exact 60-key/type/range/tuple/sentinel validation,
and safe stage evidence. The only acceptance change is to recognize a plain
record whose prototype is exactly local `Object.prototype` or exactly `null`.

V14 creates one controller-owned tab, navigates only to the fixed public token
URL, waits at most 10000 ms for the visible placeholder locator, evaluates one
bounded diagnostic, applies the two-prototype plain-record rule without
...
prototype, null-prototype, aggregate plain-record, exact-key, boolean-type,
...
the fixed safe stage booleans and capped key count. V14 PASS authorizes only offline
interpretation and a new reviewed readiness contract.

## One-shot and no-residue contract

V14 contains exactly one `tabs.new()`, one fixed `goto()`, one bounded
...
failure, timeout, truncation, tool error, or uncertainty consumes V14 and
...
Executable bytes: `20682`.

Executable SHA-256:
`6EAE11B3ACFF75C73B167B50EE3E48B861BAC5F2CCADDA5FC7FEC7C031987EF6`.

```javascript
let secureConsoleV14StructuralDiagnosticConsumed = false;
let secureConsoleV14RetainedTab = null;
await (async () => {
  const gateWasFresh = secureConsoleV14StructuralDiagnosticConsumed === false;
  ...
  try {
    ...
    secureConsoleV14RetainedTab = tab = await secureConsoleChromeV5.tabs.new();
    ...
    result = "V14_STRUCTURAL_DIAGNOSTIC_BODY_COMPLETE";
  } catch (error) {
  ...
  if (
    ...
    result = "EXACT_V14_STRUCTURAL_DIAGNOSTIC_PASS";
  }
  ...
  nodeRepl.write({
    ...
    retainedExactHandle: secureConsoleV14RetainedTab !== null,
    ...counters,
    errorClass,
    consumed: secureConsoleV14StructuralDiagnosticConsumed,
...
Exact success is `EXACT_V14_STRUCTURAL_DIAGNOSTIC_PASS` with completed new,
...
`resultPrototypeObjectExact` and `resultPrototypeNullExact` true; every
...
V14 PASS authorizes only offline interpretation and a fresh independently
reviewed readiness replacement. The visible predicate, page evaluator, and all
non-prototype validation remain unchanged. Only exact null-prototype plain-record
acceptance is added; the no-mutation boundary remains unchanged. V4 prestart and provider mutation remain
...
evaluator, exact two-prototype plain-record acceptance, unchanged remaining
