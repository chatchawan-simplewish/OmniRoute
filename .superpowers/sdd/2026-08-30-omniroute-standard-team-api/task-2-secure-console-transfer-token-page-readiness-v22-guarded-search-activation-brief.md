# OmniRoute V22 guarded explicit-search-activation readiness replacement brief
...
V20, V19, V18, V17, V16, V3, or any earlier gate. V21 was never evaluated and
must remain absent from the persistent browser context.
...
value, then the candidate-set hidden wait did not fulfill. V21 proposed one
Enter activation but was rejected before execution:
...
- review path:
  `.superpowers/sdd/2026-08-30-omniroute-standard-team-api/task-2-secure-console-transfer-token-page-readiness-v21-explicit-search-activation-sol-review.md`;
- review bytes: `11248`;
- review SHA-256:
  `952BBA8F9C4ADAA72FF71F6D42393CEB18BB73CB515E17E4781A35269CF4491A`;
- review blob: `ada9fdf5bf1be7cb3a3741dce1b5454a8c4ed3bf`;
- unresolved HIGH: `V21-001`, unproven implicit/default form action before
  the post-Enter URL check.

V21 is static failed-review evidence only. It is unconsumed and is not retried,
continued, or reinterpreted.

## Design decision

V22 keeps every V20 check and the one exact page-local keyboard hypothesis, but
adds a synchronous fixed-key prepress guard on the exact locator. Immediately
before Enter, the guard must prove in one read that the input remains connected,
visible, exact-valued, and unique in the same depth-three section; its type is
`search` or `text`; `input.form === null`; its associated-form submitter
count is therefore exactly zero; and the section still has zero Create-like
actions.

Only after that complete guard may V22 call exactly one
`tokenFilter.press("Enter", { timeoutMs: 5000 })`. It immediately rereads the
exact tab URL and requires it to remain
`https://dash.cloudflare.com/profile/api-tokens`. Any guard failure, press
rejection, navigation, URL mismatch, ambiguity, timeout, or malformed response
spends V22, closes the exact tab when possible, and stops.

Only after the fulfilled Enter and exact URL recheck does V22 run the inherited
dynamic visible-candidate hidden wait and synchronous terminal snapshot. The
Enter targets no form-associated control or button and is not authority to
submit, create, edit, delete, copy, paste, read a credential, or mutate
...
V22 preserves the V20 fresh-tab, navigation, exact structural baseline,
...
V22 requires the exact consumed-failed-clean V20 through V16
null/ineligible/no-detach/no-read predecessor bindings, all earlier retained
null bindings, the exact V4 state, and every V21 declaration absent. The
...
## One-shot V22 executable

```javascript
let secureConsoleOwnedTaskTabV22 = null;
let secureConsoleOwnedTaskTabV22Eligible = false;
let secureConsoleOwnedTaskTabV22State = "UNCREATED";
let secureConsoleOwnedTaskTabV22PreCreateDetachConsumed = false;
let secureConsoleOwnedTaskTabV22PostNativeDetachConsumed = false;
let secureConsoleCloudflareReadsV22Consumed = false;
let secureConsoleV22ReadinessConsumed = false;
await (async () => {
  const gateWasFresh = secureConsoleV22ReadinessConsumed === false;
  secureConsoleV22ReadinessConsumed = true;
  const counters = {
  ...
  let postfillInputFormNull = false;
  let postfillInputTypeSearchOrText = false;
  let postfillAssociatedFormSubmitterCount = -1;
  let postfillSectionDepth = -1;
  let postfillSectionCreateActionableCount = -1;
  ...
  try {
    declarationShape =
      ...
      typeof secureConsoleV22ReadinessConsumed === "boolean";
    ...
    predecessorStateExact =
      ...
      secureConsoleOwnedTaskTabV22 === null &&
      secureConsoleOwnedTaskTabV22Eligible === false &&
      secureConsoleOwnedTaskTabV22State === "UNCREATED" &&
      secureConsoleOwnedTaskTabV22PreCreateDetachConsumed === false &&
      secureConsoleOwnedTaskTabV22PostNativeDetachConsumed === false &&
      secureConsoleCloudflareReadsV22Consumed === false;
    if (!predecessorStateExact) throw new Error("ReadinessPredecessorStateError");

    counters.newAttempted++;
    secureConsoleOwnedTaskTabV22 = tab = await secureConsoleChromeV5.tabs.new();
    ...
    controllerOwnership = secureConsoleOwnedTaskTabV22 === tab;
    tabShape =
      controllerOwnership &&
    ...
    const untrustedQueryState = await tokenFilter.evaluate((input) => {
      ...
      return {
        ...
        inputFormNull: input.form === null,
        inputTypeSearchOrText: input.type === "search" || input.type === "text",
        associatedFormSubmitterCount: input.form === null ? 0 : -1,
        sectionDepth,
    ...
    const trustedQueryState = trustedProjection(
      ...
      [
        ...
        "inputFormNull", "inputTypeSearchOrText",
      ],
      [
        "associatedFormSubmitterCount", "sectionDepth",
        "queryInputExactCount", "sectionCreateActionableCount",
      ],
    );
    ...
    postfillInputFormNull = trustedQueryState.inputFormNull;
    postfillInputTypeSearchOrText = trustedQueryState.inputTypeSearchOrText;
    postfillAssociatedFormSubmitterCount =
      trustedQueryState.associatedFormSubmitterCount;
    postfillSectionDepth = trustedQueryState.sectionDepth;
    postfillSectionCreateActionableCount =
      trustedQueryState.sectionCreateActionableCount;
    if (trustedQueryState.inputConnected !== true ||
        trustedQueryState.inputVisible !== true ||
        trustedQueryState.inputValueExact !== true ||
        postfillInputFormNull !== true ||
        postfillInputTypeSearchOrText !== true ||
        postfillAssociatedFormSubmitterCount !== 0 ||
        postfillSectionDepth !== 3 ||
        postfillQueryInputExactCount !== 1 ||
        postfillSectionCreateActionableCount !== 0) {
      throw new Error("ReadinessPrepressSafetySemanticError");
    }
    ...
    result = "V22_TOKEN_PAGE_SEMANTIC_READINESS_BODY_COMPLETE";
  } catch (error) {
    errorClass = safeErrorClass(error);
  }

  if (result === "V22_TOKEN_PAGE_SEMANTIC_READINESS_BODY_COMPLETE") {
    secureConsoleOwnedTaskTabV22 = tab;
    secureConsoleOwnedTaskTabV22Eligible = true;
    secureConsoleOwnedTaskTabV22State = "TOKEN_PAGE_SEMANTIC_READY_ELIGIBLE_V22";
    cleanupState = "SUCCESS_TAB_RETAINED";
    residueConverged = true;
    result = "EXACT_V22_TOKEN_PAGE_SEMANTIC_READINESS_PASS";
  } else if (counters.newAttempted === 1 && counters.newFulfilled === 0) {
    cleanupState = "NEW_REJECTED_RESIDUE_UNPROVEN";
    residueConverged = false;
    secureConsoleOwnedTaskTabV22State = "V22_READINESS_FAILED_RESIDUE_UNPROVEN";
    ...
    if (typeof tab.close !== "function") {
      ...
      secureConsoleOwnedTaskTabV22 = tab;
      secureConsoleOwnedTaskTabV22Eligible = false;
      secureConsoleOwnedTaskTabV22State = "V22_READINESS_FAILED_TAB_RETAINED";
    } else {
      try {
        ...
        secureConsoleOwnedTaskTabV22 = null;
        secureConsoleOwnedTaskTabV22Eligible = false;
        secureConsoleOwnedTaskTabV22State = "V22_READINESS_FAILED_CLEAN";
  ...
  nodeRepl.write({
    ...
    postfillInputFormNull,
    postfillInputTypeSearchOrText,
    postfillAssociatedFormSubmitterCount,
    postfillSectionDepth,
    postfillSectionCreateActionableCount,
    createControlCount,
    tokenNameCount,
    matchingRowCount,
    ...
    cleanupState,
    residueConverged,
    retainedExactHandle: secureConsoleOwnedTaskTabV22 !== null,
    eligible: secureConsoleOwnedTaskTabV22Eligible,
    stateExact:
      secureConsoleOwnedTaskTabV22State === "TOKEN_PAGE_SEMANTIC_READY_ELIGIBLE_V22",
    ...counters,
    errorClass,
    consumed: secureConsoleV22ReadinessConsumed,
...
Exact PASS retains only the exact V22-created tab and marks only that handle
eligible for a separately committed and independently reviewed prestart read
contract. Every other disposition consumes V22 and follows the reviewed
...
persistent V20-through-V4 bindings, and every V22 declaration absent.

Independent Sol High review must verify the V20 incident and V21 failed-review
boundaries; exact V21 declaration absence; exact input ownership, value, type,
form-null, associated-submitter-zero, depth-three section, and
Create-actionable-zero guard immediately before Enter; one Enter only;
immediate exact URL recheck; dynamic candidate-set settle semantics;
synchronous terminal snapshot; every inherited signature/completeness
constraint; output/cardinality; retention/cleanup; no
secret/provider-persistent action; no retry; and both mandatory confirmation
boundaries.
