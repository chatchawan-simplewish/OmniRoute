...
const briefPath = path.join(
  directory,
  "task-2-secure-console-transfer-v39-fresh-session-cross-realm-task-tab-reacquisition-brief.md",
);
...
assert.equal(occurrences("secureConsoleSetupBrowserRuntimeV39();"), 1);
assert.equal(occurrences('browsers.get("chrome")'), 1);
assert.equal(occurrences("secureConsoleChromeV39.documentation()"), 1);
assert.equal(occurrences("secureConsoleChromeV39.nameSession("), 1);
assert.equal(occurrences("secureConsoleChromeV39.user.openTabs()"), 1);
assert.equal(occurrences("secureConsoleChromeV39.user.claimTab(candidate.id)"), 1);
...
const targetUrl = `https://dash.cloudflare.com/${account}/mysw.me`;
...
assert.deepEqual(trustedListing([{ id: "other", url: "https://example.invalid/" }]), {
  count: 1,
  targetCount: 0,
  candidate: null,
});
const multiple = trustedListing([
  { id: "task-1", url: targetUrl },
  { id: "task-2", url: `${targetUrl}/tokens` },
]);
assert.equal(multiple.targetCount, 2);
assert.equal(multiple.candidate, null);
...
assert.equal(trustedListing([nullRecord]).targetCount, 1);
...
const transformedCell = cell.replace(
  importSource,
  "const imported = globalThis.__v39FixtureImported;",
);
...
const runTransformedCell = async () => {
  ...
  urlCalls = 0;
  snapshotCalls = 0;
  globalThis.__v39FixtureImported = fixtureImported;
  globalThis.nodeRepl = {
    async write(value) {
  ...
  try {
    ...
    delete globalThis.__v39FixtureImported;
...
assert.equal(success.targetCandidateCount, 1);
...
assert.equal(success.state, "V39_ACCOUNT_HOME_READY_ELIGIBLE");
...
const assertFixedFailure = async (thrown) => {
  ...
  assert.equal(failure.result,
    "V39_FRESH_CROSS_REALM_TASK_REACQUISITION_FAILED_STOP");
  ...
  assert.equal(failure.state, "V39_REACQUISITION_OR_READINESS_FAILED");
  assert.equal(claimCalls, 1);
