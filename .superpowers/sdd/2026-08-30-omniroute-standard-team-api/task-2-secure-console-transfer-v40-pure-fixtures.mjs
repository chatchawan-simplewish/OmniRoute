...
const briefPath = path.join(
  directory,
  "task-2-secure-console-transfer-v40-fresh-session-ordered-selected-tab-reacquisition-brief.md",
);
...
assert.equal(occurrences("secureConsoleSetupBrowserRuntimeV40();"), 1);
assert.equal(occurrences('browsers.get("chrome")'), 1);
assert.equal(occurrences("secureConsoleChromeV40.documentation()"), 1);
assert.equal(occurrences("secureConsoleChromeV40.nameSession("), 1);
assert.equal(occurrences("secureConsoleChromeV40.user.openTabs()"), 1);
assert.equal(occurrences("secureConsoleChromeV40.user.claimTab(candidate.id)"), 1);
...
const transformedCell = cell.replace(
  importSource,
  "const imported = globalThis.__v40FixtureImported;",
);
...
const runTransformedCell = async () => {
  ...
  urlCalls = 0;
  snapshotCalls = 0;
  globalThis.__v40FixtureImported = fixtureImported;
  globalThis.nodeRepl = {
    async write(value) {
  ...
  try {
    ...
    delete globalThis.__v40FixtureImported;
...
assert.equal(success.state, "V40_ACCOUNT_HOME_READY_ELIGIBLE");
...
const assertFixedFailure = async (thrown) => {
  ...
  assert.equal(failure.result,
    "V40_FRESH_CROSS_REALM_TASK_REACQUISITION_FAILED_STOP");
  ...
  assert.equal(failure.state, "V40_REACQUISITION_OR_READINESS_FAILED");
  assert.equal(claimCalls, 1);
