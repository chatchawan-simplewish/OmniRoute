...
const executablePath = path.join(
  directory,
  "task-2-secure-console-transfer-v44-fresh-realm-token-page-readiness-executable.js",
);
...
const listingBlock = executable.match(
  /\/\/ BEGIN_V44Attachment_PURE_TRUSTED_LISTING\n([\s\S]*?)\n\s*\/\/ END_V44Attachment_PURE_TRUSTED_LISTING/,
);
...
const observationHook =
  "    globalThis.__v44Fixture.observed = {\n" +
  "      consumed: secureConsoleV44Consumed,\n" +
  "      tabNull: secureConsoleOwnedTaskTabV44 === null,\n" +
  "      eligible: secureConsoleOwnedTaskTabV44Eligible,\n" +
  "      state: secureConsoleOwnedTaskTabV44State,\n" +
  "      attachmentTabNull: secureConsoleOwnedTaskTabV44Attachment === null,\n" +
  "      attachmentEligible: secureConsoleOwnedTaskTabV44AttachmentEligible,\n" +
  "      attachmentState: secureConsoleV44AttachmentState,\n" +
  "      chromeNull: secureConsoleChromeV44Attachment === null,\n" +
  "      agentNull: secureConsoleAgentV44Attachment === null,\n" +
  "      setupNull: secureConsoleSetupBrowserRuntimeV44Attachment === null,\n" +
  "      evidenceNull: secureConsoleV44AttachmentEvidence === null,\n" +
  "      outputErrorNull: secureConsoleV44TerminalOutputFailure === null,\n" +
  "      counters: { ...counters },\n" +
  "    };\n";
const documentationFailureHook =
  "  globalThis.__v44Fixture.documentationFailureObserved = {\n" +
  "    consumed: secureConsoleV44Consumed,\n" +
  "    tabNull: secureConsoleOwnedTaskTabV44 === null,\n" +
  "    eligible: secureConsoleOwnedTaskTabV44Eligible,\n" +
  "    state: secureConsoleOwnedTaskTabV44State,\n" +
...
const instrumented = executable
  .replace(importSource, "const imported = globalThis.__v44Fixture.imported;")
  .replace(
    ...
    "if (secureConsoleV44TerminalOutputFailure !== null) {\n" +
      "    const terminalOutputFailure = secureConsoleV44TerminalOutputFailure;\n" +
      "    globalThis.__v44Fixture.documentationFailureAttachment =\n" +
  ...
  .replace(
    ...
    "      globalThis.__v44Fixture.finalWriteAttachment =\n" +
      "        { ...secureConsoleV44AttachmentEvidence };\n" +
      "      await nodeRepl.write({",
  )
  .replace(
...
const run = async (options = {}, prelude = "", source = instrumented) => {
  ...
  globalThis.__v44Fixture = fixture;
  const nodeReplPrelude = `
const nodeRepl = {
  async write(value) {
    globalThis.__v44Fixture.writeCalls++;
    if (globalThis.__v44Fixture.failWriteAt === globalThis.__v44Fixture.writeCalls) {
      throw globalThis.__v44Fixture.outputThrown;
    }
    globalThis.__v44Fixture.writes.push(value);
  },
};
`;
  let caught = null;
  try {
    ...
    delete globalThis.__v44Fixture;
  }
...
assert.equal(success.result, "EXACT_V44_TOKEN_PAGE_SEMANTIC_READINESS_PASS");
assert.equal(
...
assert.deepEqual(successRun.fixture.observed, {
  ...
  attachmentState: "V44Attachment_TRANSFERRED_TO_V44",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
...
for (const prelude of [
  ...
  assert.equal(contaminated.output.result, "V44_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
for (const [before, after] of [
  ["let secureConsoleOwnedTaskTabV44 = null;", "let secureConsoleOwnedTaskTabV44 = {};"],
  ["let secureConsoleOwnedTaskTabV44Eligible = false;", "let secureConsoleOwnedTaskTabV44Eligible = true;"],
  ["let secureConsoleOwnedTaskTabV44State = \"UNADOPTED\";", "let secureConsoleOwnedTaskTabV44State = \"DIRTY\";"],
  ["let secureConsoleOwnedTaskTabV44PreCreateDetachConsumed = false;", "let secureConsoleOwnedTaskTabV44PreCreateDetachConsumed = true;"],
  ["let secureConsoleOwnedTaskTabV44PostNativeDetachConsumed = false;", "let secureConsoleOwnedTaskTabV44PostNativeDetachConsumed = true;"],
  ["let secureConsoleCloudflareReadsV44Consumed = false;", "let secureConsoleCloudflareReadsV44Consumed = true;"],
  ["let secureConsoleSetupBrowserRuntimeV44Attachment = null;", "let secureConsoleSetupBrowserRuntimeV44Attachment = {};"],
  ["let secureConsoleAgentV44Attachment = null;", "let secureConsoleAgentV44Attachment = {};"],
  ["let secureConsoleChromeV44Attachment = null;", "let secureConsoleChromeV44Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV44Attachment = null;", "let secureConsoleOwnedTaskTabV44Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV44AttachmentEligible = false;", "let secureConsoleOwnedTaskTabV44AttachmentEligible = true;"],
  ["let secureConsoleV44AttachmentState = \"UNCREATED\";", "let secureConsoleV44AttachmentState = \"DIRTY\";"],
]) {
...
for (const stage of [
  ...
  assert.equal(failed.output.result, "V44_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP", stage);
...
const assertHostileClaimFailure = async (thrown, token) => {
  ...
  assert.equal(failureRun.output.result, "V44_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
assert.equal(finalOutputFailure.fixture.observed.state, "V44_FINAL_OUTPUT_FAILED_STOP");
...
assert.equal(
  ...
  "V44Attachment_DOWNSTREAM_OUTPUT_FAILURE_DETACHED",
);
...
assert.deepEqual(documentationOutputFailure.fixture.documentationFailureObserved, {
  ...
  attachmentTabNull: true,
  attachmentEligible: false,
  attachmentState: "V44Attachment_REACQUISITION_OR_READINESS_FAILED",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
