...
const briefPath = path.join(
  directory,
  "task-2-secure-console-transfer-v46-fresh-realm-token-page-readiness-brief.md",
);
const brief = fs.readFileSync(briefPath, "utf8");
const executablePath = path.join(
  directory,
  "task-2-secure-console-transfer-v46-fresh-realm-token-page-readiness-executable.js",
);
...
assert.equal(
  occurrences("secureConsoleChromeV46Attachment.user.claimTab(candidate.tab)"),
  1,
);
...
const listingBlock = executable.match(
  /\/\/ BEGIN_V46Attachment_PURE_TRUSTED_LISTING\n([\s\S]*?)\n\s*\/\/ END_V46Attachment_PURE_TRUSTED_LISTING/,
);
...
const observationHook =
  "    globalThis.__v46Fixture.observed = {\n" +
  "      consumed: secureConsoleV46Consumed,\n" +
  "      tabNull: secureConsoleOwnedTaskTabV46 === null,\n" +
  "      eligible: secureConsoleOwnedTaskTabV46Eligible,\n" +
  "      state: secureConsoleOwnedTaskTabV46State,\n" +
  "      attachmentTabNull: secureConsoleOwnedTaskTabV46Attachment === null,\n" +
  "      attachmentEligible: secureConsoleOwnedTaskTabV46AttachmentEligible,\n" +
  "      attachmentState: secureConsoleV46AttachmentState,\n" +
  "      chromeNull: secureConsoleChromeV46Attachment === null,\n" +
  "      agentNull: secureConsoleAgentV46Attachment === null,\n" +
  "      setupNull: secureConsoleSetupBrowserRuntimeV46Attachment === null,\n" +
  "      evidenceNull: secureConsoleV46AttachmentEvidence === null,\n" +
  "      outputErrorNull: secureConsoleV46TerminalOutputFailure === null,\n" +
  "      counters: { ...counters },\n" +
  "    };\n";
const documentationFailureHook =
  "  globalThis.__v46Fixture.documentationFailureObserved = {\n" +
  "    consumed: secureConsoleV46Consumed,\n" +
  "    tabNull: secureConsoleOwnedTaskTabV46 === null,\n" +
  "    eligible: secureConsoleOwnedTaskTabV46Eligible,\n" +
  "    state: secureConsoleOwnedTaskTabV46State,\n" +
...
const instrumented = executable
  .replace(importSource, "const imported = globalThis.__v46Fixture.imported;")
  .replace(
    ...
    "if (secureConsoleV46TerminalOutputFailure !== null) {\n" +
      "    const terminalOutputFailure = secureConsoleV46TerminalOutputFailure;\n" +
      "    globalThis.__v46Fixture.documentationFailureAttachment =\n" +
  ...
  .replace(
    ...
    "      globalThis.__v46Fixture.finalWriteAttachment =\n" +
      "        { ...secureConsoleV46AttachmentEvidence };\n" +
      "      await nodeRepl.write({",
  )
  .replace(
...
const run = async (options = {}, prelude = "", source = instrumented) => {
  ...
  globalThis.__v46Fixture = fixture;
  const nodeReplPrelude = `
const nodeRepl = {
  async write(value) {
    globalThis.__v46Fixture.writeCalls++;
    if (globalThis.__v46Fixture.failWriteAt === globalThis.__v46Fixture.writeCalls) {
      throw globalThis.__v46Fixture.outputThrown;
    }
    globalThis.__v46Fixture.writes.push(value);
  },
};
`;
  let caught = null;
  try {
    ...
    delete globalThis.__v46Fixture;
  }
...
assert.equal(success.result, "EXACT_V46_TOKEN_PAGE_SEMANTIC_READINESS_PASS");
...
assert.equal(absentUrlSuccessRun.output.result, "EXACT_V46_TOKEN_PAGE_SEMANTIC_READINESS_PASS");
...
assert.deepEqual(successRun.fixture.observed, {
  ...
  attachmentState: "V46Attachment_TRANSFERRED_TO_V46",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
...
for (const prelude of [
  ...
  assert.equal(contaminated.output.result, "V46_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
const nonfreshSource = instrumented.replace(
  ...
  "let secureConsoleV46Consumed = true;",
);
...
for (const [before, after] of [
  ["let secureConsoleOwnedTaskTabV46 = null;", "let secureConsoleOwnedTaskTabV46 = {};"],
  ["let secureConsoleOwnedTaskTabV46Eligible = false;", "let secureConsoleOwnedTaskTabV46Eligible = true;"],
  ["let secureConsoleOwnedTaskTabV46State = \"UNADOPTED\";", "let secureConsoleOwnedTaskTabV46State = \"DIRTY\";"],
  ["let secureConsoleOwnedTaskTabV46PreCreateDetachConsumed = false;", "let secureConsoleOwnedTaskTabV46PreCreateDetachConsumed = true;"],
  ["let secureConsoleOwnedTaskTabV46PostNativeDetachConsumed = false;", "let secureConsoleOwnedTaskTabV46PostNativeDetachConsumed = true;"],
  ["let secureConsoleCloudflareReadsV46Consumed = false;", "let secureConsoleCloudflareReadsV46Consumed = true;"],
  ["let secureConsoleSetupBrowserRuntimeV46Attachment = null;", "let secureConsoleSetupBrowserRuntimeV46Attachment = {};"],
  ["let secureConsoleAgentV46Attachment = null;", "let secureConsoleAgentV46Attachment = {};"],
  ["let secureConsoleChromeV46Attachment = null;", "let secureConsoleChromeV46Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV46Attachment = null;", "let secureConsoleOwnedTaskTabV46Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV46AttachmentEligible = false;", "let secureConsoleOwnedTaskTabV46AttachmentEligible = true;"],
  ["let secureConsoleV46AttachmentState = \"UNCREATED\";", "let secureConsoleV46AttachmentState = \"DIRTY\";"],
]) {
...
for (const stage of [
  ...
  assert.equal(failed.output.result, "V46_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP", stage);
...
const assertHostileClaimFailure = async (thrown, token) => {
  ...
  assert.equal(failureRun.output.result, "V46_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
assert.equal(finalOutputFailure.fixture.observed.state, "V46_FINAL_OUTPUT_FAILED_STOP");
...
assert.equal(
  ...
  "V46Attachment_DOWNSTREAM_OUTPUT_FAILURE_DETACHED",
);
...
assert.deepEqual(documentationOutputFailure.fixture.documentationFailureObserved, {
  ...
  attachmentTabNull: true,
  attachmentEligible: false,
  attachmentState: "V46Attachment_REACQUISITION_OR_READINESS_FAILED",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
