...
const briefPath = path.join(
  directory,
  "task-2-secure-console-transfer-v47-fresh-realm-token-page-readiness-brief.md",
);
...
assert.equal(
  occurrences("secureConsoleChromeV47Attachment.user.claimTab(candidate.tab)"),
  1,
);
...
const listingBlock = executable.match(
  /\/\/ BEGIN_V47Attachment_PURE_TRUSTED_LISTING\n([\s\S]*?)\n\s*\/\/ END_V47Attachment_PURE_TRUSTED_LISTING/,
);
...
const observationHook =
  "    globalThis.__v47Fixture.observed = {\n" +
  "      consumed: secureConsoleV47Consumed,\n" +
  "      tabNull: secureConsoleOwnedTaskTabV47 === null,\n" +
  "      eligible: secureConsoleOwnedTaskTabV47Eligible,\n" +
  "      state: secureConsoleOwnedTaskTabV47State,\n" +
  "      attachmentTabNull: secureConsoleOwnedTaskTabV47Attachment === null,\n" +
  "      attachmentEligible: secureConsoleOwnedTaskTabV47AttachmentEligible,\n" +
  "      attachmentState: secureConsoleV47AttachmentState,\n" +
  "      chromeNull: secureConsoleChromeV47Attachment === null,\n" +
  "      agentNull: secureConsoleAgentV47Attachment === null,\n" +
  "      setupNull: secureConsoleSetupBrowserRuntimeV47Attachment === null,\n" +
  "      evidenceNull: secureConsoleV47AttachmentEvidence === null,\n" +
  "      outputErrorNull: secureConsoleV47TerminalOutputFailure === null,\n" +
  "      counters: { ...counters },\n" +
  "    };\n";
const documentationFailureHook =
  "  globalThis.__v47Fixture.documentationFailureObserved = {\n" +
  "    consumed: secureConsoleV47Consumed,\n" +
  "    tabNull: secureConsoleOwnedTaskTabV47 === null,\n" +
  "    eligible: secureConsoleOwnedTaskTabV47Eligible,\n" +
  "    state: secureConsoleOwnedTaskTabV47State,\n" +
...
const instrumented = executable
  .replace(importSource, "const imported = globalThis.__v47Fixture.imported;")
  .replace(
    ...
    "if (secureConsoleV47TerminalOutputFailure !== null) {\n" +
      "    const terminalOutputFailure = secureConsoleV47TerminalOutputFailure;\n" +
      "    globalThis.__v47Fixture.documentationFailureAttachment =\n" +
  ...
  .replace(
    ...
    "      globalThis.__v47Fixture.finalWriteAttachment =\n" +
      "        { ...secureConsoleV47AttachmentEvidence };\n" +
      "      await nodeRepl.write({",
  )
  .replace(
...
const run = async (options = {}, prelude = "", source = instrumented) => {
  ...
  globalThis.__v47Fixture = fixture;
  const nodeReplPrelude = `
const nodeRepl = {
  async write(value) {
    globalThis.__v47Fixture.writeCalls++;
    if (globalThis.__v47Fixture.failWriteAt === globalThis.__v47Fixture.writeCalls) {
      throw globalThis.__v47Fixture.outputThrown;
    }
    globalThis.__v47Fixture.writes.push(value);
  },
};
`;
  let caught = null;
  try {
    ...
    delete globalThis.__v47Fixture;
  }
...
assert.equal(success.result, "EXACT_V47_TOKEN_PAGE_SEMANTIC_READINESS_PASS");
...
assert.equal(absentUrlSuccessRun.output.result, "EXACT_V47_TOKEN_PAGE_SEMANTIC_READINESS_PASS");
...
assert.deepEqual(successRun.fixture.observed, {
  ...
  attachmentState: "V47Attachment_TRANSFERRED_TO_V47",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
...
const nonfreshSource = instrumented.replace(
  ...
  "let secureConsoleV47Consumed = true;",
);
...
for (const [before, after] of [
  ["let secureConsoleOwnedTaskTabV47 = null;", "let secureConsoleOwnedTaskTabV47 = {};"],
  ["let secureConsoleOwnedTaskTabV47Eligible = false;", "let secureConsoleOwnedTaskTabV47Eligible = true;"],
  ["let secureConsoleOwnedTaskTabV47State = \"UNADOPTED\";", "let secureConsoleOwnedTaskTabV47State = \"DIRTY\";"],
  ["let secureConsoleOwnedTaskTabV47PreCreateDetachConsumed = false;", "let secureConsoleOwnedTaskTabV47PreCreateDetachConsumed = true;"],
  ["let secureConsoleOwnedTaskTabV47PostNativeDetachConsumed = false;", "let secureConsoleOwnedTaskTabV47PostNativeDetachConsumed = true;"],
  ["let secureConsoleCloudflareReadsV47Consumed = false;", "let secureConsoleCloudflareReadsV47Consumed = true;"],
  ["let secureConsoleSetupBrowserRuntimeV47Attachment = null;", "let secureConsoleSetupBrowserRuntimeV47Attachment = {};"],
  ["let secureConsoleAgentV47Attachment = null;", "let secureConsoleAgentV47Attachment = {};"],
  ["let secureConsoleChromeV47Attachment = null;", "let secureConsoleChromeV47Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV47Attachment = null;", "let secureConsoleOwnedTaskTabV47Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV47AttachmentEligible = false;", "let secureConsoleOwnedTaskTabV47AttachmentEligible = true;"],
  ["let secureConsoleV47AttachmentState = \"UNCREATED\";", "let secureConsoleV47AttachmentState = \"DIRTY\";"],
]) {
...
for (const stage of [
  ...
  assert.equal(failed.output.result, "V47_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP", stage);
...
const assertHostileClaimFailure = async (thrown, token) => {
  ...
  assert.equal(failureRun.output.result, "V47_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
assert.equal(finalOutputFailure.fixture.observed.state, "V47_FINAL_OUTPUT_FAILED_STOP");
...
assert.equal(
  ...
  "V47Attachment_DOWNSTREAM_OUTPUT_FAILURE_DETACHED",
);
...
assert.deepEqual(documentationOutputFailure.fixture.documentationFailureObserved, {
  ...
  attachmentTabNull: true,
  attachmentEligible: false,
  attachmentState: "V47Attachment_REACQUISITION_OR_READINESS_FAILED",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
