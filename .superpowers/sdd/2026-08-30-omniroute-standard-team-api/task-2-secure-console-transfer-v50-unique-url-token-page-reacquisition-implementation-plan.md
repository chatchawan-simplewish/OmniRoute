# V50 unique-URL token-page reacquisition implementation plan
...
- Source executable: final committed V49 executable
- Source fixture: final committed V49 pure fixture
- New predecessor boundary: all seven persistent V49 declarations
...
Mechanically rename final V49 ownership, state, result, and marker literals to
V50. Preserve all downstream browser, account-home, token-page, counter,
output, privacy, retention, failure-cleanup, terminal-output, and one-proof-only
semantics.

## Task 2 — Implement the sole semantic delta

Extend the fresh-realm guard through the seven V49 globals. Validate the exact
...
1. Node syntax validation of the V50 executable;
2. the V50 pure fixture, requiring `V50_PURE_FIXTURES_PASS`;
3. a direct normalized executable comparison to final V49 after removing only
...
5. Git status/index checks proving the 12 product paths are unchanged and only
   the four V50 candidate paths enter the index.
