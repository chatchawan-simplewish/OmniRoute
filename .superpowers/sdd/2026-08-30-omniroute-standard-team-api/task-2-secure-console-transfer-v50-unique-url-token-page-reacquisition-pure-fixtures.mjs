...
assert.equal(
  occurrences("secureConsoleChromeV50Attachment.user.claimTab(candidate.tab)"),
  1,
);
...
const listingBlock = executable.match(
  /\/\/ BEGIN_V50Attachment_PURE_TRUSTED_LISTING\n([\s\S]*?)\n\s*\/\/ END_V50Attachment_PURE_TRUSTED_LISTING/,
);
...
const observationHook =
  "    globalThis.__v50Fixture.observed = {\n" +
  "      consumed: secureConsoleV50Consumed,\n" +
  "      tabNull: secureConsoleOwnedTaskTabV50 === null,\n" +
  "      eligible: secureConsoleOwnedTaskTabV50Eligible,\n" +
  "      state: secureConsoleOwnedTaskTabV50State,\n" +
  "      attachmentTabNull: secureConsoleOwnedTaskTabV50Attachment === null,\n" +
  "      attachmentEligible: secureConsoleOwnedTaskTabV50AttachmentEligible,\n" +
  "      attachmentState: secureConsoleV50AttachmentState,\n" +
  "      chromeNull: secureConsoleChromeV50Attachment === null,\n" +
  "      agentNull: secureConsoleAgentV50Attachment === null,\n" +
  "      setupNull: secureConsoleSetupBrowserRuntimeV50Attachment === null,\n" +
  "      evidenceNull: secureConsoleV50AttachmentEvidence === null,\n" +
  "      outputErrorNull: secureConsoleV50TerminalOutputFailure === null,\n" +
  "      counters: { ...counters },\n" +
  "    };\n";
const documentationFailureHook =
  "  globalThis.__v50Fixture.documentationFailureObserved = {\n" +
  "    consumed: secureConsoleV50Consumed,\n" +
  "    tabNull: secureConsoleOwnedTaskTabV50 === null,\n" +
  "    eligible: secureConsoleOwnedTaskTabV50Eligible,\n" +
  "    state: secureConsoleOwnedTaskTabV50State,\n" +
...
const instrumented = executable
  .replace(importSource, "const imported = globalThis.__v50Fixture.imported;")
  .replace(
    ...
    "if (secureConsoleV50TerminalOutputFailure !== null) {\n" +
      "    const terminalOutputFailure = secureConsoleV50TerminalOutputFailure;\n" +
      "    globalThis.__v50Fixture.documentationFailureAttachment =\n" +
  ...
  .replace(
    ...
    "      globalThis.__v50Fixture.finalWriteAttachment =\n" +
      "        { ...secureConsoleV50AttachmentEvidence };\n" +
      "      await nodeRepl.write({",
  )
  .replace(
...
const run = async (options = {}, prelude = "", source = instrumented) => {
  ...
  globalThis.__v50Fixture = fixture;
  const nodeReplPrelude = `
const nodeRepl = {
  async write(value) {
    globalThis.__v50Fixture.writeCalls++;
    if (globalThis.__v50Fixture.failWriteAt === globalThis.__v50Fixture.writeCalls) {
      throw globalThis.__v50Fixture.outputThrown;
    }
    globalThis.__v50Fixture.writes.push(value);
  },
};
`;
  let caught = null;
  try {
    ...
    delete globalThis.__v50Fixture;
  }
...
assert.equal(success.result, "EXACT_V50_TOKEN_PAGE_SEMANTIC_READINESS_PASS");
...
assert.deepEqual(successRun.fixture.observed, {
  ...
  attachmentState: "V50Attachment_TRANSFERRED_TO_V50",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
...
for (const prelude of [
  ...
  assert.equal(contaminated.output.result, "V50_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
const nonfreshSource = instrumented.replace(
  ...
  "let secureConsoleV50Consumed = true;",
);
...
for (const [before, after] of [
  ["let secureConsoleOwnedTaskTabV50 = null;", "let secureConsoleOwnedTaskTabV50 = {};"],
  ["let secureConsoleOwnedTaskTabV50Eligible = false;", "let secureConsoleOwnedTaskTabV50Eligible = true;"],
  ["let secureConsoleOwnedTaskTabV50State = \"UNADOPTED\";", "let secureConsoleOwnedTaskTabV50State = \"DIRTY\";"],
  ["let secureConsoleOwnedTaskTabV50PreCreateDetachConsumed = false;", "let secureConsoleOwnedTaskTabV50PreCreateDetachConsumed = true;"],
  ["let secureConsoleOwnedTaskTabV50PostNativeDetachConsumed = false;", "let secureConsoleOwnedTaskTabV50PostNativeDetachConsumed = true;"],
  ["let secureConsoleCloudflareReadsV50Consumed = false;", "let secureConsoleCloudflareReadsV50Consumed = true;"],
  ["let secureConsoleSetupBrowserRuntimeV50Attachment = null;", "let secureConsoleSetupBrowserRuntimeV50Attachment = {};"],
  ["let secureConsoleAgentV50Attachment = null;", "let secureConsoleAgentV50Attachment = {};"],
  ["let secureConsoleChromeV50Attachment = null;", "let secureConsoleChromeV50Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV50Attachment = null;", "let secureConsoleOwnedTaskTabV50Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV50AttachmentEligible = false;", "let secureConsoleOwnedTaskTabV50AttachmentEligible = true;"],
  ["let secureConsoleV50AttachmentState = \"UNCREATED\";", "let secureConsoleV50AttachmentState = \"DIRTY\";"],
]) {
...
for (const stage of [
  ...
  assert.equal(failed.output.result, "V50_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP", stage);
...
const assertHostileClaimFailure = async (thrown, token) => {
  ...
  assert.equal(failureRun.output.result, "V50_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
assert.equal(finalOutputFailure.fixture.observed.state, "V50_FINAL_OUTPUT_FAILED_STOP");
...
assert.equal(
  ...
  "V50Attachment_DOWNSTREAM_OUTPUT_FAILURE_DETACHED",
);
...
assert.deepEqual(documentationOutputFailure.fixture.documentationFailureObserved, {
  ...
  attachmentTabNull: true,
  attachmentEligible: false,
  attachmentState: "V50Attachment_REACQUISITION_OR_READINESS_FAILED",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
...
assert.ok(brief.includes("D60CC7898F509013D19DFF1E7254065F5C2FF531386211A348CDF392F3D59988"));
