...
assert.equal(
  occurrences("secureConsoleChromeV51Attachment.user.claimTab(candidate.tab)"),
  1,
);
...
const listingBlock = executable.match(
  /\/\/ BEGIN_V51Attachment_PURE_TRUSTED_LISTING\n([\s\S]*?)\n\s*\/\/ END_V51Attachment_PURE_TRUSTED_LISTING/,
);
...
const observationHook =
  "    globalThis.__v51Fixture.observed = {\n" +
  "      consumed: secureConsoleV51Consumed,\n" +
  "      tabNull: secureConsoleOwnedTaskTabV51 === null,\n" +
  "      eligible: secureConsoleOwnedTaskTabV51Eligible,\n" +
  "      state: secureConsoleOwnedTaskTabV51State,\n" +
  "      attachmentTabNull: secureConsoleOwnedTaskTabV51Attachment === null,\n" +
  "      attachmentEligible: secureConsoleOwnedTaskTabV51AttachmentEligible,\n" +
  "      attachmentState: secureConsoleV51AttachmentState,\n" +
  "      chromeNull: secureConsoleChromeV51Attachment === null,\n" +
  "      agentNull: secureConsoleAgentV51Attachment === null,\n" +
  "      setupNull: secureConsoleSetupBrowserRuntimeV51Attachment === null,\n" +
  "      evidenceNull: secureConsoleV51AttachmentEvidence === null,\n" +
  "      outputErrorNull: secureConsoleV51TerminalOutputFailure === null,\n" +
  "      counters: { ...counters },\n" +
  "    };\n";
const documentationFailureHook =
  "  globalThis.__v51Fixture.documentationFailureObserved = {\n" +
  "    consumed: secureConsoleV51Consumed,\n" +
  "    tabNull: secureConsoleOwnedTaskTabV51 === null,\n" +
  "    eligible: secureConsoleOwnedTaskTabV51Eligible,\n" +
  "    state: secureConsoleOwnedTaskTabV51State,\n" +
...
const instrumented = executable
  .replace(importSource, "const imported = globalThis.__v51Fixture.imported;")
  .replace(
    "if (secureConsoleV51TerminalOutputFailure !== null) {\n" +
      "    const terminalOutputFailure = secureConsoleV51TerminalOutputFailure;\n" +
      "    secureConsoleV51TerminalOutputFailure = null;\n" +
      "    secureConsoleV51AttachmentEvidence = null;\n" +
      "    throw terminalOutputFailure;",
    "if (secureConsoleV51TerminalOutputFailure !== null) {\n" +
      "    const terminalOutputFailure = secureConsoleV51TerminalOutputFailure;\n" +
      "    globalThis.__v51Fixture.documentationFailureAttachment =\n" +
      "      { ...secureConsoleV51AttachmentEvidence };\n" +
      "    secureConsoleV51TerminalOutputFailure = null;\n" +
      "    secureConsoleV51AttachmentEvidence = null;\n" +
  ...
  .replace(
    ...
    "      globalThis.__v51Fixture.finalWriteAttachment =\n" +
      "        { ...secureConsoleV51AttachmentEvidence };\n" +
      "      await nodeRepl.write({",
  )
  .replace(
...
const run = async (options = {}, prelude = "", source = instrumented) => {
  ...
  globalThis.__v51Fixture = fixture;
  const nodeReplPrelude = `
const nodeRepl = {
  async write(value) {
    globalThis.__v51Fixture.writeCalls++;
    if (globalThis.__v51Fixture.failWriteAt === globalThis.__v51Fixture.writeCalls) {
      throw globalThis.__v51Fixture.outputThrown;
    }
    globalThis.__v51Fixture.writes.push(value);
  },
};
`;
  let caught = null;
  try {
    ...
    delete globalThis.__v51Fixture;
  }
...
assert.equal(success.result, "EXACT_V51_TOKEN_PAGE_SEMANTIC_READINESS_PASS");
...
assert.equal(
  ...
  "EXACT_V51Attachment_CROSS_REALM_TASK_TAB_ACCOUNT_HOME_REACQUISITION_PASS",
);
...
assert.deepEqual(successRun.fixture.observed, {
  ...
  attachmentState: "V51Attachment_TRANSFERRED_TO_V51",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
...
for (const prelude of [
  ...
  assert.equal(contaminated.output.result, "V51_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
  ...
  assert.equal(
    ...
    "V51_TOKEN_PAGE_SEMANTIC_READINESS_FAILED",
  );
...
const nonfreshSource = instrumented.replace(
  ...
  "let secureConsoleV51Consumed = true;",
);
...
for (const [before, after] of [
  ["let secureConsoleOwnedTaskTabV51 = null;", "let secureConsoleOwnedTaskTabV51 = {};"],
  ["let secureConsoleOwnedTaskTabV51Eligible = false;", "let secureConsoleOwnedTaskTabV51Eligible = true;"],
  ["let secureConsoleOwnedTaskTabV51State = \"UNADOPTED\";", "let secureConsoleOwnedTaskTabV51State = \"DIRTY\";"],
  ["let secureConsoleOwnedTaskTabV51PreCreateDetachConsumed = false;", "let secureConsoleOwnedTaskTabV51PreCreateDetachConsumed = true;"],
  ["let secureConsoleOwnedTaskTabV51PostNativeDetachConsumed = false;", "let secureConsoleOwnedTaskTabV51PostNativeDetachConsumed = true;"],
  ["let secureConsoleCloudflareReadsV51Consumed = false;", "let secureConsoleCloudflareReadsV51Consumed = true;"],
  ["let secureConsoleSetupBrowserRuntimeV51Attachment = null;", "let secureConsoleSetupBrowserRuntimeV51Attachment = {};"],
  ["let secureConsoleAgentV51Attachment = null;", "let secureConsoleAgentV51Attachment = {};"],
  ["let secureConsoleChromeV51Attachment = null;", "let secureConsoleChromeV51Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV51Attachment = null;", "let secureConsoleOwnedTaskTabV51Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV51AttachmentEligible = false;", "let secureConsoleOwnedTaskTabV51AttachmentEligible = true;"],
  ["let secureConsoleV51AttachmentState = \"UNCREATED\";", "let secureConsoleV51AttachmentState = \"DIRTY\";"],
]) {
...
for (const stage of [
  ...
  assert.equal(failed.output.result, "V51_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP", stage);
...
const assertHostileClaimFailure = async (thrown, token) => {
  ...
  assert.equal(failureRun.output.result, "V51_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
assert.equal(finalOutputFailure.fixture.observed.state, "V51_FINAL_OUTPUT_FAILED_STOP");
...
assert.equal(
  ...
  "V51Attachment_DOWNSTREAM_OUTPUT_FAILURE_DETACHED",
);
...
assert.deepEqual(documentationOutputFailure.fixture.documentationFailureObserved, {
  ...
  attachmentTabNull: true,
  attachmentEligible: false,
  attachmentState: "V51Attachment_REACQUISITION_OR_READINESS_FAILED",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
