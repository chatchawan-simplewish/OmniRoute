...
const documentationPath = "C:/Users/chatc/.codex/plugins/cache/openai-bundled/chrome/26.901.20858/docs/api.json";
const pinnedFile = (filePath) => {
  const bytes = fs.readFileSync(filePath);
  return { bytes: bytes.length, sha256: crypto.createHash("sha256").update(bytes).digest("hex").toUpperCase() };
};
assert.deepEqual(pinnedFile(runtimePath), { bytes: 150611, sha256: "B9B9BC2319D5EE6AA0B1E481D63BB2130D28102FC7C9080803AB5552185D9037" });
assert.deepEqual(pinnedFile(documentationPath), { bytes: 59294, sha256: "FC7966FFBC9010252AD3EA745E061068BEC3919EFFF860A87E6013A38A7E277F" });
assert.ok(brief.includes("10619"));
assert.ok(brief.includes("89D36435A31AE04E560A27D53D8A0953F19E837DF60837FADCF3DC174C0B9477"));
assert.match(executable, /secureConsoleV52SourceProjection = Object\.freeze\(\{\n  records: 10619,\n  sha256: "89D36435A31AE04E560A27D53D8A0953F19E837DF60837FADCF3DC174C0B9477",\n\}\);/);
...
assert.equal(
  occurrences("secureConsoleChromeV52Attachment.user.claimTab(candidate.tab)"),
  1,
);
...
const listingBlock = executable.match(
  /\/\/ BEGIN_V52Attachment_PURE_TRUSTED_LISTING\n([\s\S]*?)\n\s*\/\/ END_V52Attachment_PURE_TRUSTED_LISTING/,
);
assert.notEqual(listingBlock, null);
const trustedListing = new Function(
  `${listingBlock[1]}\nreturn trustedListing;`,
)();
const listingTargetUrl = "https://dash.cloudflare.com/profile/api-tokens";
const listingSummary = (value) => value === null ? null : {
  id: value.id,
  count: value.count,
  urlPresent: value.urlPresent,
  urlExact: value.urlExact,
};
const validTarget = { id: "task-tab", url: listingTargetUrl };
const validListing = {
  id: "task-tab",
  count: 5,
  urlPresent: true,
...
const validListingResult = trustedListing([
  { id: "absent-url" },
  { id: "http-tab", url: "http://dash.cloudflare.com/profile/api-tokens" },
  { id: "internal-tab", url: "chrome://settings/" },
  { id: "ported-tab", url: "https://dash.cloudflare.com:8443/profile/api-tokens" },
  validTarget,
]);
assert.deepEqual(listingSummary(validListingResult), validListing);
assert.equal(validListingResult.tab, validTarget);
...
assert.deepEqual(listingSummary(foreignListingResult), {
  ...validListing,
  count: 2,
});
assert.equal(foreignListingResult.tab, foreignListing[1]);
...
assert.equal(trustedListing([absentUrlRecord]), null);
...
assert.equal(trustedListing([
  validTarget,
  { id: "duplicate-target", url: listingTargetUrl },
]), null);
...
assert.equal(nullRecordResult.urlExact, true);
...
symbolRecord[Symbol("unexpected")] = true;
class ListingRecord {
  constructor() {
    this.id = "task-tab";
...
const rejectedListings = [
  ...
  ["array-symbol", symbolListing],
  ["index-accessor", indexAccessor],
  ["record-symbol", makeListing(symbolRecord)],
  ["record-accessor", [validTarget, recordAccessor]],
  ["later-record-accessor", [validTarget, laterHostileRecord]],
  ["record-class", makeListing(new ListingRecord())],
  ["record-primitive", ["task-tab"]],
  ["unknown-key", makeListing(validRecord({ extra: "unexpected" }))],
  ["non-enumerable-field", makeListing(nonEnumerableFieldRecord)],
  ["non-enumerable-slot", nonEnumerableSlot],
  ...fieldBoundaryListings,
  ["zero-exact-match", makeListing(validRecord({ url: "not a URL" }))],
  ["duplicate-exact-match", [validRecord(), validRecord({ id: "duplicate" })]],
];
...
const observationHook =
  "    globalThis.__v52Fixture.observed = {\n" +
  "      consumed: secureConsoleV52Consumed,\n" +
  "      tabNull: secureConsoleOwnedTaskTabV52 === null,\n" +
  "      eligible: secureConsoleOwnedTaskTabV52Eligible,\n" +
  "      state: secureConsoleOwnedTaskTabV52State,\n" +
  "      attachmentTabNull: secureConsoleOwnedTaskTabV52Attachment === null,\n" +
  "      attachmentEligible: secureConsoleOwnedTaskTabV52AttachmentEligible,\n" +
  "      attachmentState: secureConsoleV52AttachmentState,\n" +
  "      chromeNull: secureConsoleChromeV52Attachment === null,\n" +
  "      agentNull: secureConsoleAgentV52Attachment === null,\n" +
  "      setupNull: secureConsoleSetupBrowserRuntimeV52Attachment === null,\n" +
  "      evidenceNull: secureConsoleV52AttachmentEvidence === null,\n" +
  "      outputErrorNull: secureConsoleV52TerminalOutputFailure === null,\n" +
  "      counters: { ...counters },\n" +
  "    };\n";
const documentationFailureHook =
  "  globalThis.__v52Fixture.documentationFailureObserved = {\n" +
  "    consumed: secureConsoleV52Consumed,\n" +
  "    tabNull: secureConsoleOwnedTaskTabV52 === null,\n" +
  "    eligible: secureConsoleOwnedTaskTabV52Eligible,\n" +
  "    state: secureConsoleOwnedTaskTabV52State,\n" +
...
const instrumented = executable
  .replace(importSource, "const imported = globalThis.__v52Fixture.imported;")
  .replace(
    "if (secureConsoleV52TerminalOutputFailure !== null) {\n" +
      "    const terminalOutputFailure = secureConsoleV52TerminalOutputFailure;\n" +
      "    secureConsoleV52TerminalOutputFailure = null;\n" +
      "    secureConsoleV52AttachmentEvidence = null;\n" +
      "    throw terminalOutputFailure;",
    "if (secureConsoleV52TerminalOutputFailure !== null) {\n" +
      "    const terminalOutputFailure = secureConsoleV52TerminalOutputFailure;\n" +
      "    globalThis.__v52Fixture.documentationFailureAttachment =\n" +
      "      { ...secureConsoleV52AttachmentEvidence };\n" +
      "    secureConsoleV52TerminalOutputFailure = null;\n" +
      "    secureConsoleV52AttachmentEvidence = null;\n" +
  ...
  .replace(
    ...
    "      globalThis.__v52Fixture.finalWriteAttachment =\n" +
      "        { ...secureConsoleV52AttachmentEvidence };\n" +
      "      await nodeRepl.write({",
  )
  .replace(
  ...
  const browser = {
    ...
    user: {
      async openTabs() {
        ...
        const offered = vm.runInNewContext(`[
          { id: "other-tab", url: "https://example.invalid/" },
          { id: "task-tab", url: "${listingTargetUrl}" }
        ]`);
        fixture.offeredCandidate = offered[1];
        return offered;
      },
      async claimTab(tabInfo) {
        fixture.claimCalls++;
        assert.equal(tabInfo, fixture.offeredCandidate);
...
const run = async (options = {}, prelude = "", source = instrumented) => {
  ...
  globalThis.__v52Fixture = fixture;
  const nodeReplPrelude = `
const nodeRepl = {
  async write(value) {
    globalThis.__v52Fixture.writeCalls++;
    if (globalThis.__v52Fixture.failWriteAt === globalThis.__v52Fixture.writeCalls) {
      throw globalThis.__v52Fixture.outputThrown;
    }
    globalThis.__v52Fixture.writes.push(value);
  },
};
`;
  let caught = null;
  try {
    ...
    delete globalThis.__v52Fixture;
  }
...
const expectedAttachmentKeys = [
  "agentShape", "bindingEligible", "bindingNull", "candidateUrlExact",
  "candidateUrlPresent",
  ...
  "openTabsAttempted", "openTabsFulfilled", "uniqueUrlSelected", "result",
...
assert.equal(success.result, "EXACT_V52_TOKEN_PAGE_SEMANTIC_READINESS_PASS");
assert.equal(success.attachment.candidateUrlPresent, true);
assert.equal(success.attachment.candidateUrlExact, true);
const serializedSuccess = JSON.stringify(success);
for (const privateValue of ["task-tab", "other-tab", listingTargetUrl]) {
  assert.equal(serializedSuccess.includes(privateValue), false, privateValue);
}
assert.equal(
  success.attachment.result,
  "EXACT_V52Attachment_CROSS_REALM_TASK_TAB_ACCOUNT_HOME_REACQUISITION_PASS",
);
...
assert.deepEqual(successRun.fixture.observed, {
  ...
  attachmentState: "V52Attachment_TRANSFERRED_TO_V52",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
...
for (const prelude of [
  ...
  "let secureConsoleOwnedTaskTabV49 = null;\n",
  "let secureConsoleOwnedTaskTabV49Eligible = false;\n",
  "let secureConsoleOwnedTaskTabV49State = \"DIRTY\";\n",
  "let secureConsoleOwnedTaskTabV49PreCreateDetachConsumed = false;\n",
  ...
  "let secureConsoleOwnedTaskTabV51 = null;\n",
  "let secureConsoleOwnedTaskTabV51Eligible = false;\n",
  "let secureConsoleOwnedTaskTabV51State = \"DIRTY\";\n",
  "let secureConsoleOwnedTaskTabV51PreCreateDetachConsumed = false;\n",
  "let secureConsoleOwnedTaskTabV51PostNativeDetachConsumed = false;\n",
  "let secureConsoleCloudflareReadsV51Consumed = false;\n",
  "let secureConsoleV51Consumed = false;\n",
]) {
  ...
  assert.equal(contaminated.output.result, "V52_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
  ...
  assert.equal(
    ...
    "V52_TOKEN_PAGE_SEMANTIC_READINESS_FAILED",
  );
...
const nonfreshSource = instrumented.replace(
  "let secureConsoleV52Consumed = false;",
  "let secureConsoleV52Consumed = true;",
);
...
for (const [before, after] of [
  ["let secureConsoleOwnedTaskTabV52 = null;", "let secureConsoleOwnedTaskTabV52 = {};"],
  ["let secureConsoleOwnedTaskTabV52Eligible = false;", "let secureConsoleOwnedTaskTabV52Eligible = true;"],
  ["let secureConsoleOwnedTaskTabV52State = \"UNADOPTED\";", "let secureConsoleOwnedTaskTabV52State = \"DIRTY\";"],
  ["let secureConsoleOwnedTaskTabV52PreCreateDetachConsumed = false;", "let secureConsoleOwnedTaskTabV52PreCreateDetachConsumed = true;"],
  ["let secureConsoleOwnedTaskTabV52PostNativeDetachConsumed = false;", "let secureConsoleOwnedTaskTabV52PostNativeDetachConsumed = true;"],
  ["let secureConsoleCloudflareReadsV52Consumed = false;", "let secureConsoleCloudflareReadsV52Consumed = true;"],
  ["let secureConsoleSetupBrowserRuntimeV52Attachment = null;", "let secureConsoleSetupBrowserRuntimeV52Attachment = {};"],
  ["let secureConsoleAgentV52Attachment = null;", "let secureConsoleAgentV52Attachment = {};"],
  ["let secureConsoleChromeV52Attachment = null;", "let secureConsoleChromeV52Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV52Attachment = null;", "let secureConsoleOwnedTaskTabV52Attachment = {};"],
  ["let secureConsoleOwnedTaskTabV52AttachmentEligible = false;", "let secureConsoleOwnedTaskTabV52AttachmentEligible = true;"],
  ["let secureConsoleV52AttachmentState = \"UNCREATED\";", "let secureConsoleV52AttachmentState = \"DIRTY\";"],
]) {
...
for (const stage of [
  ...
  assert.equal(failed.output.result, "V52_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP", stage);
...
const assertHostileClaimFailure = async (thrown, token) => {
  ...
  assert.equal(failureRun.output.result, "V52_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
assert.equal(finalOutputFailure.fixture.observed.state, "V52_FINAL_OUTPUT_FAILED_STOP");
...
assert.equal(
  ...
  "V52Attachment_DOWNSTREAM_OUTPUT_FAILURE_DETACHED",
);
...
assert.deepEqual(documentationOutputFailure.fixture.documentationFailureObserved, {
  ...
  attachmentTabNull: true,
  attachmentEligible: false,
  attachmentState: "V52Attachment_REACQUISITION_OR_READINESS_FAILED",
  chromeNull: true,
  agentNull: true,
  setupNull: true,
  evidenceNull: true,
...
assert.ok(brief.includes("B9B9BC2319D5EE6AA0B1E481D63BB2130D28102FC7C9080803AB5552185D9037"));
assert.ok(brief.includes("FC7966FFBC9010252AD3EA745E061068BEC3919EFFF860A87E6013A38A7E277F"));
