...
const designPath = path.join(
  directory,
  "task-2-secure-console-transfer-v53-direct-cell-replacement-design.md",
);
const design = fs.readFileSync(designPath, "utf8");
const executablePath = path.join(
  directory,
  "task-2-secure-console-transfer-v53-direct-cell-replacement-executable.js",
);
...
assert.ok(design.includes("10619"));
assert.ok(design.includes("89D36435A31AE04E560A27D53D8A0953F19E837DF60837FADCF3DC174C0B9477"));
assert.match(candidate, /records:10619,sha256:"89D36435A31AE04E560A27D53D8A0953F19E837DF60837FADCF3DC174C0B9477"/);
...
assert.equal(
  occurrences("secureConsoleChromeV53Attachment.user.claimTab(candidate.tab)"),
  1,
);
...
const observationHook =
  "    globalThis.__v53Fixture.observed = {\n" +
  "      consumed: secureConsoleV53Consumed,\n" +
  "      tabNull: secureConsoleOwnedTaskTabV53 === null,\n" +
  "      eligible: secureConsoleOwnedTaskTabV53Eligible,\n" +
  "      state: secureConsoleOwnedTaskTabV53State,\n" +
  "      attachmentTabNull: secureConsoleOwnedTaskTabV53Attachment === null,\n" +
  "      attachmentEligible: secureConsoleOwnedTaskTabV53AttachmentEligible,\n" +
  "      attachmentState: secureConsoleV53AttachmentState,\n" +
  "      chromeNull: secureConsoleChromeV53Attachment === null,\n" +
  "      agentNull: secureConsoleAgentV53Attachment === null,\n" +
  "      setupNull: secureConsoleSetupBrowserRuntimeV53Attachment === null,\n" +
...
const documentationFailureHook =
  "  globalThis.__v53Fixture.documentationFailureObserved = {\n" +
  "    consumed: secureConsoleV53Consumed,\n" +
  "    tabNull: secureConsoleOwnedTaskTabV53 === null,\n" +
  "    eligible: secureConsoleOwnedTaskTabV53Eligible,\n" +
  "    state: secureConsoleOwnedTaskTabV53State,\n" +
...
let instrumented = executable
  ...
  .replace(
    "if (secureConsoleV53TerminalOutputFailure !== null) {\n" +
      ...
      "    secureConsoleV53TerminalOutputFailure = null;\n" +
      "    secureConsoleV53AttachmentEvidence = null;\n" +
      "    throw terminalOutputFailure;",
    "if (secureConsoleV53TerminalOutputFailure !== null) {\n" +
      "    const terminalOutputFailure = secureConsoleV53TerminalOutputFailure;\n" +
      "    globalThis.__v53Fixture.documentationFailureAttachment =\n" +
      "      { ...secureConsoleV53AttachmentEvidence };\n" +
      "    secureConsoleV53TerminalOutputFailure = null;\n" +
      "    secureConsoleV53AttachmentEvidence = null;\n" +
  ...
  .replace(
    ...
    "      globalThis.__v53Fixture.finalWriteAttachment =\n" +
      "        { ...secureConsoleV53AttachmentEvidence };\n" +
      "      await nodeRepl.write({",
  )
  .replace(
...
const nodeRepl = {
  async write(value) {
    globalThis.__v53Fixture.writeCalls++;
    if (globalThis.__v53Fixture.failWriteAt === globalThis.__v53Fixture.writeCalls) {
      throw globalThis.__v53Fixture.outputThrown;
    }
    globalThis.__v53Fixture.writes.push(value);
  },
};
`;
  let caught = null;
  try {
    ...
    delete globalThis.__v53Fixture;
  }
...
assert.equal(
  ...
  "EXACT_V53Attachment_CROSS_REALM_TASK_TAB_ACCOUNT_HOME_REACQUISITION_PASS",
);
...
).matchAll(/typeof (secureConsole[A-Za-z0-9]+) === "undefined"/g)].map((match) => match[1]);
assert.equal(predecessorNames.length, 118);
/* Historical sampled list retained as provenance for V52. */
/*
...
for (const prelude of predecessorNames.map((name) => `let ${name} = false;\n`)) {
  ...
  assert.equal(
    ...
    "V53_TOKEN_PAGE_SEMANTIC_READINESS_FAILED",
  );
...
for (const stage of [
  ...
  assert.equal(failed.output.result, "V53_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP", stage);
...
const assertHostileClaimFailure = async (thrown, token) => {
  ...
  assert.equal(failureRun.output.result, "V53_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
console.log(JSON.stringify({
  ...
  designSha256: crypto.createHash("sha256").update(design).digest("hex").toUpperCase(),
  executableBytes,
  executableSha256,
  fixtureBytes,
  fixtureSha256,
