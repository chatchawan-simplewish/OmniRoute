...
const executablePath = path.join(directory,
  "task-2-secure-console-transfer-v56-disk-module-executable.mjs");
const designPath = path.join(directory,
  "task-2-secure-console-transfer-v56-disk-module-design.md");
const loaderPath = path.join(directory,
  "task-2-secure-console-transfer-v56-disk-module-loader-cell.js");
const v55SourcePath = path.join(directory,
...
const v55Source = fs.readFileSync(v55SourcePath, "utf8").replace(/\r\n/g, "\n");
const sha256 = (value) => crypto.createHash("sha256").update(value).digest("hex").toUpperCase();

const persistentBindings = [
  "secureConsoleOwnedTaskTabV56", "secureConsoleOwnedTaskTabV56Eligible",
  "secureConsoleOwnedTaskTabV56State", "secureConsoleV56Consumed",
...
`;
assert.equal(source, v55Source.replaceAll("V55", "V56") + sourceExport);
assert.equal(source.includes("V55"), false);
...
const terminalNeedle = `secureConsoleV56AttachmentEvidence = null;
...
const terminalProbeSource = source.replace(terminalNeedle,
  `await nodeRepl.capture({tabNull:secureConsoleOwnedTaskTabV56===null,eligible:secureConsoleOwnedTaskTabV56Eligible,state:secureConsoleOwnedTaskTabV56State,preCreate:secureConsoleOwnedTaskTabV56PreCreateDetachConsumed,postNative:secureConsoleOwnedTaskTabV56PostNativeDetachConsumed,cloudflareReads:secureConsoleCloudflareReadsV56Consumed,consumed:secureConsoleV56Consumed,attachmentBindingNull:secureConsoleOwnedTaskTabV56Attachment===null,attachmentEligible:secureConsoleOwnedTaskTabV56AttachmentEligible,chromeNull:secureConsoleChromeV56Attachment===null,agentNull:secureConsoleAgentV56Attachment===null,setupNull:secureConsoleSetupBrowserRuntimeV56Attachment===null,downstreamCounters:{...counters},attachmentCounters:secureConsoleV56AttachmentEvidence});
...
const documentationNeedle = `secureConsoleV56AttachmentEvidence = null;
...
const documentationProbeSource = source.replace(documentationNeedle,
  `await nodeRepl.capture({tabNull:secureConsoleOwnedTaskTabV56===null,eligible:secureConsoleOwnedTaskTabV56Eligible,state:secureConsoleOwnedTaskTabV56State,preCreate:secureConsoleOwnedTaskTabV56PreCreateDetachConsumed,postNative:secureConsoleOwnedTaskTabV56PostNativeDetachConsumed,cloudflareReads:secureConsoleCloudflareReadsV56Consumed,consumed:secureConsoleV56Consumed,attachmentBindingNull:secureConsoleOwnedTaskTabV56Attachment===null,attachmentEligible:secureConsoleOwnedTaskTabV56AttachmentEligible,chromeNull:secureConsoleChromeV56Attachment===null,agentNull:secureConsoleAgentV56Attachment===null,setupNull:secureConsoleSetupBrowserRuntimeV56Attachment===null,attachmentCounters:secureConsoleV56AttachmentEvidence});
    secureConsoleV56AttachmentEvidence = null;
    throw terminalOutputFailure;`);
...
const ordinaryProbeSource = source.replace(ordinaryNeedle,
  `    await nodeRepl.capture({tabNull:secureConsoleOwnedTaskTabV56===null,eligible:secureConsoleOwnedTaskTabV56Eligible,state:secureConsoleOwnedTaskTabV56State,preCreate:secureConsoleOwnedTaskTabV56PreCreateDetachConsumed,postNative:secureConsoleOwnedTaskTabV56PostNativeDetachConsumed,cloudflareReads:secureConsoleCloudflareReadsV56Consumed,consumed:secureConsoleV56Consumed,attachmentBindingNull:secureConsoleOwnedTaskTabV56Attachment===null,attachmentEligible:secureConsoleOwnedTaskTabV56AttachmentEligible,chromeNull:secureConsoleChromeV56Attachment===null,agentNull:secureConsoleAgentV56Attachment===null,setupNull:secureConsoleSetupBrowserRuntimeV56Attachment===null});
...
assert.equal(success.output.result, "EXACT_V56_TOKEN_PAGE_SEMANTIC_READINESS_PASS");
...
const moduleFixture = makeFixture();
globalThis.__v56FixtureModule = moduleFixture.module;
globalThis.__v56NodeRepl = moduleFixture.nodeRepl;
const moduleCandidate = candidate
  .replace(importLiteral, "Promise.resolve(globalThis.__v56FixtureModule)")
  .replaceAll("nodeRepl", "globalThis.__v56NodeRepl");
let moduleNamespace;
try {
  moduleNamespace = await import(`data:text/javascript;base64,${
    Buffer.from(moduleCandidate).toString("base64")}#v56-export-proof`);
} finally {
  delete globalThis.__v56FixtureModule;
  delete globalThis.__v56NodeRepl;
}
assert.deepEqual(Object.keys(moduleNamespace).sort(), [...persistentBindings].sort());
assert.equal(moduleNamespace.secureConsoleOwnedTaskTabV56 === null, false);
assert.equal(moduleNamespace.secureConsoleOwnedTaskTabV56Eligible, true);
assert.equal(moduleNamespace.secureConsoleOwnedTaskTabV56State,
  "TOKEN_PAGE_SEMANTIC_READY_ELIGIBLE");
assert.equal(moduleNamespace.secureConsoleOwnedTaskTabV56PreCreateDetachConsumed, false);
assert.equal(moduleNamespace.secureConsoleOwnedTaskTabV56PostNativeDetachConsumed, false);
assert.equal(moduleNamespace.secureConsoleCloudflareReadsV56Consumed, false);
assert.equal(moduleNamespace.secureConsoleV56Consumed, true);
assert.equal("__v56FixtureModule" in globalThis, false);
assert.equal("__v56NodeRepl" in globalThis, false);
const linkOnlySuccess = await run({ createButtonCount: 0, createLinkCount: 1 });
assert.equal(linkOnlySuccess.output.result, "EXACT_V56_TOKEN_PAGE_SEMANTIC_READINESS_PASS");
...
assert.equal(staleSuccess.output.result, "EXACT_V56_TOKEN_PAGE_SEMANTIC_READINESS_PASS");
...
for (const resetFailure of [
  ...
  assert.equal(failed.output.result, "V56_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
assert.equal(visibleSentinel.output.result, "V56_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP");
...
for (const overrides of failureCases) {
  ...
  for (const [key, value] of Object.entries({
    result: "V56_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP", errorClass: "Error",
...
for (const throwAt of throwStages) {
  ...
  assert.equal(failed.output.result, "V56_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP", throwAt);
...
for (const name of predecessorNames) {
  ...
  assert.equal(contaminated.output.result, "V56_TOKEN_PAGE_SEMANTIC_READINESS_FAILED_STOP", name);
...
assert.equal(documentationFailureProbe.attachmentCounters.result,
  "V56Attachment_FRESH_CROSS_REALM_TASK_REACQUISITION_FAILED_STOP");
...
assert.equal(outputFailureProbe.state, "V56_FINAL_OUTPUT_FAILED_STOP");
...
const loaderAsciiFailure = await runLoader({ bytes: nonAsciiBytes,
  hashOverride: expectedModuleSha256 });
const loaderReadFailure = await runLoader({ readError: true });
const loaderImportFailure = await runLoader({ importError: true });
for (const failed of [loaderByteFailure, loaderHashFailure, loaderAsciiFailure,
  loaderReadFailure, loaderImportFailure]) {
  assert.ok(failed.caught instanceof Error);
  assert.equal(failed.state.secureConsoleV56Module, null);
}
for (const failed of [loaderByteFailure, loaderHashFailure, loaderAsciiFailure,
  loaderReadFailure]) assert.equal(failed.imports.length, 0);
assert.equal(loaderImportFailure.imports.length, 1);
