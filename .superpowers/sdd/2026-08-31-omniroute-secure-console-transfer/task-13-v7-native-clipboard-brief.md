# Task 13 OmniRoute native clipboard preflight v7 brief
...
- current base / Task 12 PASS classification:
  `ce26c909a259b17ce02768b813d3563c872c580e`;
- corrected v6 pattern: `18717` bytes, SHA-256
  `3D5670C7E5E5C3D58205D168311E4732A605325CF9A082C1AD84EEE862B1CFFB`;
- Task 12 cleanup report commit
  `b2ae6e1a3340581ef2c96af1d891c9246eb6a1d1`: `1720` bytes,
  SHA-256
  `A13C62C4F9702D03BD674530FCE294A1FBFCC2E9357FFE5B50442FE0191118D2`;
...
Task 12 independently proves zero Chrome tabs, current clipboard empty, and the
old v6 retained challenge null. V7 uses fresh bindings and the existing
connected `residualV5Chrome` binding. Old v3/v5 listener/process residuals
...
intervening live action, and never-declared fresh v7 bindings.
...
'PREFLIGHT_V7_BASELINE_CLEAR_ATTEMPTED=' + $clearAttempted
'PREFLIGHT_V7_BASELINE_CLEAR_FULFILLED=' + $clearFulfilled
'PREFLIGHT_V7_BASELINE_READ_ATTEMPTED=' + $readAttempted
'PREFLIGHT_V7_BASELINE_READ_FULFILLED=' + $readFulfilled
'PREFLIGHT_V7_BASELINE_EMPTY=' + $empty.ToString().ToUpperInvariant()
'PREFLIGHT_V7_BASELINE_ERROR=' + $errorClass
...
```javascript
let nativeClipboardPreflightV7ExpectedChallenge = null;
let nativeClipboardPreflightV7Call3Eligible = false;
let nativeClipboardPreflightV7RetainedTab = null;
await (async () => {
  ...
  try {
    ...
    await residualV5Chrome.nameSession("omniroute-v7-native-clipboard");
    counters.sessionNameFulfilled++;
    sessionNameUncertain = false;
    const { randomBytes } = await import("node:crypto");
    const challenge = `OMNI-PREFLIGHT-V7-${randomBytes(16).toString("hex").toUpperCase()}`;
    challengeShape = /^OMNI-PREFLIGHT-V7-[0-9A-F]{32}$/.test(challenge);
    ...
    nativeClipboardPreflightV7RetainedTab = tab;
    tabState = "OPEN";
    ...
    await nativeClipboardPreflightV7RetainedTab.clipboard.writeText(challenge);
    counters.clipboardWriteFulfilled++;
    tabState = "WRITTEN";
    tabState = "CLOSE_UNCERTAIN";
    counters.browserCloseAttempted++;
    await nativeClipboardPreflightV7RetainedTab.close();
    counters.browserCloseFulfilled++;
    nativeClipboardPreflightV7RetainedTab = null;
    tabState = "CLOSED";
    if (challengeShape && challengeLength === 50 && Object.values(counters).every((value) => value === 1) && !sessionNameUncertain && !browserUncertain) {
      nativeClipboardPreflightV7ExpectedChallenge = challenge;
      nativeClipboardPreflightV7Call3Eligible = true;
    ...
    if (result !== "NATIVE_WRITE_SETTLED_TAB_CLOSED") {
      nativeClipboardPreflightV7Call3Eligible = false;
    }
    nodeRepl.write({
      result, errorClass, challengeShape, challengeLength, counters, tabState,
      exactTabClosed: tabState === "CLOSED" && nativeClipboardPreflightV7RetainedTab === null,
      retainedTabBinding: nativeClipboardPreflightV7RetainedTab !== null,
      browserUncertain,
...
```javascript
await (async () => {
  const result = { result: "HANDOFF_UNCERTAIN_RETAINED_BINDING", errorClass: "NONE", childStartAttempted: 0, childExitFulfilled: 0, childExitCode: null, stdoutSchemaValid: false, comparison: null, challengeBindingRetained: nativeClipboardPreflightV7ExpectedChallenge !== null };
  const safeClass = (value, fallback) => /^[A-Za-z][A-Za-z0-9_.-]{0,63}$/.test(value) ? value : fallback;
  const consumedEligibility = nativeClipboardPreflightV7Call3Eligible === true;
  nativeClipboardPreflightV7Call3Eligible = false;
  if (!consumedEligibility || typeof nativeClipboardPreflightV7ExpectedChallenge !== "string" || !/^OMNI-PREFLIGHT-V7-[0-9A-F]{32}$/.test(nativeClipboardPreflightV7ExpectedChallenge)) {
    result.result = "HANDOFF_BINDING_INVALID_RETAINED_AS_FOUND";
    result.challengeBindingRetained = nativeClipboardPreflightV7ExpectedChallenge !== null;
...
try {
    ...
    if ($null -eq $expected -or $expected -cnotmatch '^OMNI-PREFLIGHT-V7-[0-9A-F]{32}$' -or $expected.Length -ne 50) { $comparisonError='HANDOFF_SHAPE_FAIL' }
    else {
        $comparisonReads++
        $observed=[string](Get-Clipboard -Raw)
        $comparisonMatch=$observed -ceq $expected
        $observedShape=$observed -cmatch '^OMNI-PREFLIGHT-V7-[0-9A-F]{32}$'
...
'PREFLIGHT_V7_HANDOFF_READS='+$handoffReads
'PREFLIGHT_V7_COMPARISON_READS='+$comparisonReads
'PREFLIGHT_V7_COMPARISON_MATCH='+$comparisonMatch.ToString().ToUpperInvariant()
'PREFLIGHT_V7_OBSERVED_SHAPE='+$observedShape.ToString().ToUpperInvariant()
'PREFLIGHT_V7_OBSERVED_LENGTH='+$observedLength
'PREFLIGHT_V7_COMPARISON_ERROR='+$comparisonError
'PREFLIGHT_V7_FINAL_CLEAR_ATTEMPTED='+$finalClearAttempted
'PREFLIGHT_V7_FINAL_CLEAR_FULFILLED='+$finalClearFulfilled
'PREFLIGHT_V7_FINAL_READ_ATTEMPTED='+$finalReadAttempted
'PREFLIGHT_V7_FINAL_READ_FULFILLED='+$finalReadFulfilled
'PREFLIGHT_V7_FINAL_EMPTY='+$finalEmpty.ToString().ToUpperInvariant()
'PREFLIGHT_V7_CLEANUP_ERROR='+$cleanupError
'PREFLIGHT_V7_RESULT='+$(if($pass){'PASS'}else{'FAIL'})
if($pass){exit 0}; exit 2`;
  ...
  const keys = ["PREFLIGHT_V7_HANDOFF_READS","PREFLIGHT_V7_COMPARISON_READS","PREFLIGHT_V7_COMPARISON_MATCH","PREFLIGHT_V7_OBSERVED_SHAPE","PREFLIGHT_V7_OBSERVED_LENGTH","PREFLIGHT_V7_COMPARISON_ERROR","PREFLIGHT_V7_FINAL_CLEAR_ATTEMPTED","PREFLIGHT_V7_FINAL_CLEAR_FULFILLED","PREFLIGHT_V7_FINAL_READ_ATTEMPTED","PREFLIGHT_V7_FINAL_READ_FULFILLED","PREFLIGHT_V7_FINAL_EMPTY","PREFLIGHT_V7_CLEANUP_ERROR","PREFLIGHT_V7_RESULT"];
  const parsed = {};
  ...
  if (schema) result.comparison = { handoffReads:Number(parsed.PREFLIGHT_V7_HANDOFF_READS), comparisonReads:Number(parsed.PREFLIGHT_V7_COMPARISON_READS), comparisonMatch:parsed.PREFLIGHT_V7_COMPARISON_MATCH, observedShape:parsed.PREFLIGHT_V7_OBSERVED_SHAPE, observedLength:Number(parsed.PREFLIGHT_V7_OBSERVED_LENGTH), comparisonError:parsed.PREFLIGHT_V7_COMPARISON_ERROR, finalClearAttempted:Number(parsed.PREFLIGHT_V7_FINAL_CLEAR_ATTEMPTED), finalClearFulfilled:Number(parsed.PREFLIGHT_V7_FINAL_CLEAR_FULFILLED), finalReadAttempted:Number(parsed.PREFLIGHT_V7_FINAL_READ_ATTEMPTED), finalReadFulfilled:Number(parsed.PREFLIGHT_V7_FINAL_READ_FULFILLED), finalEmpty:parsed.PREFLIGHT_V7_FINAL_EMPTY, cleanupError:parsed.PREFLIGHT_V7_CLEANUP_ERROR, powerShellResult:parsed.PREFLIGHT_V7_RESULT };
...
no later action. Browser uncertainty may retain the exact v7 tab binding for a
