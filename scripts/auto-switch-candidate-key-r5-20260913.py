...
CONTRACT = ROOT / "docs/auto-switch-candidate-key-r5-contract-20260913.md"
RESULT = ROOT / "docs/auto-switch-candidate-key-r5-result-20260913.md"
...
OPERATOR_STORE = "/root/.omniroute-qualification/auto-switch-candidate-qualification-r5-20260913.key"
EXECUTION_READY = False
FIXTURE_CANDIDATE_ID = "f" * 64
RESULT_SCHEMA = "auto-switch-candidate-key-r5-result/v1"
PREFLIGHT_SCHEMA = "auto-switch-candidate-key-r5-preflight/v1"
RESULT_LIMIT = 16384
PREFLIGHT_CHECKS = (
    "candidate_identity", "live_identity", "image_identity", "volume_identity", "cloud_false",
    "migration_170", "migration_171", "schema_columns", "credential_shape", "server_env_exact",
    "store_absent", "key_api_schema", "key_name_absent", "combo_api_schema",
...
def require_bound():
    ...
    if not isinstance(PREFLIGHT_RESULT, pathlib.Path) or not re.fullmatch(r"[0-9a-f]{64}", PREFLIGHT_RESULT_SHA256 or ""):
        missing.append("preflight_result")
    return missing
...
const data=fs.mkdtempSync(path.join(os.tmpdir(),'omniroute-key-r5-roundtrip-'));
...
try{
 ...
 const db=core.getDbInstance(),dbrow=db.prepare('SELECT id,ip_allowlist FROM api_keys WHERE id=?').get(created.id);
 const migration_rows=db.prepare("SELECT version,name FROM _omniroute_migrations WHERE version IN ('170','171') ORDER BY version").all();mismatch_fields.sort();
 emit(JSON.stringify({create_status:createRes.status,patch_status:patchRes.status,get_status:getRes.status,missing_fields,extra_fields,mismatch_fields,db_ip_allowlist_is_null:!!dbrow&&dbrow.id===created.id&&dbrow.ip_allowlist===null,migration_rows}));
 if(createRes.status!==201||patchRes.status!==200||getRes.status!==200||missing_fields.length||extra_fields.length||mismatch_fields.length||!dbrow||dbrow.ip_allowlist!==null||JSON.stringify(migration_rows)!==JSON.stringify([{version:'170',name:'agent_route_runs'},{version:'171',name:'agent_route_deferred_metrics'}]))process.exitCode=1;
...
async function main(){
 ...
 equal({modelAccessMode:row.modelAccessMode,allowedModels:row.allowedModels,blockedModels:row.blockedModels,allowedCombos:row.allowedCombos,allowedConnections:row.allowedConnections,allowedQuotas:row.allowedQuotas,noLog:row.noLog,autoResolve:row.autoResolve,isActive:row.isActive,accessSchedule:row.accessSchedule,maxRequestsPerDay:row.maxRequestsPerDay,maxRequestsPerMinute:row.maxRequestsPerMinute,throttleDelayMs:row.throttleDelayMs,rateLimits:row.rateLimits,maxSessions:row.maxSessions,revokedAt:row.revokedAt,expiresAt:row.expiresAt,lastUsedAt:row.lastUsedAt,ipAllowlist:row.ipAllowlist,scopes:row.scopes,isBanned:row.isBanned,proxyId:row.proxyId,allowedEndpoints:row.allowedEndpoints,streamDefaultMode:row.streamDefaultMode,cacheDefaultMode:row.cacheDefaultMode,disableNonPublicModels:row.disableNonPublicModels,allowUsageCommand:row.allowUsageCommand,usageLimitEnabled:row.usageLimitEnabled,dailyUsageLimitUsd:row.dailyUsageLimitUsd,weeklyUsageLimitUsd:row.weeklyUsageLimitUsd,chaosModeEnabled:row.chaosModeEnabled,compressionEnabled:row.compressionEnabled},{modelAccessMode:'restricted',allowedModels:MODELS,blockedModels:[],allowedCombos:[SENTINEL],allowedConnections:CONNECTIONS,allowedQuotas:[],noLog:true,autoResolve:false,isActive:true,accessSchedule:null,maxRequestsPerDay:null,maxRequestsPerMinute:null,throttleDelayMs:0,rateLimits:null,maxSessions:0,revokedAt:null,expiresAt:null,lastUsedAt:null,ipAllowlist:null,scopes:['agent:route'],isBanned:false,proxyId:null,allowedEndpoints:['chat','models'],streamDefaultMode:'legacy',cacheDefaultMode:'legacy',disableNonPublicModels:false,allowUsageCommand:false,usageLimitEnabled:false,dailyUsageLimitUsd:null,weeklyUsageLimitUsd:null,chaosModeEnabled:false,compressionEnabled:true});
  process.stdout.write(JSON.stringify({status:'KEY_NATIVE_R5_PASS',key_id:keyId,secret,codex_inactive_readback:true}));
}
main().catch(async e=>{if(e&&['validation','http_4xx','http_5xx','http_other','timeout','connection','output_limit'].includes(e.category))failure=e.category;await rollback();process.stdout.write(JSON.stringify({status:'KEY_NATIVE_R5_STOP',stage,key_id:keyId,failure_category:failure,http_status:httpStatus,rollback_attempted:rollbackAttempted,key_inactive_readback:keyInactiveReadback}));process.exitCode=1});
'''

PREFLIGHT_NODE_TEMPLATE = r'''
const http=require('node:http');
const ADMIN=Buffer.from('__ADMIN_B64__','base64').toString('utf8');
const NAME=__NAME__,CODEX=__CODEX__,SENTINEL=__SENTINEL__;
...
let stage='key_api_schema';
function exact(v,w){return !!v&&typeof v==='object'&&!Array.isArray(v)&&JSON.stringify(Object.keys(v).sort())===JSON.stringify([...w].sort())}
function request(path){return new Promise((resolve,reject)=>{const q=http.request({host:'127.0.0.1',port:20128,method:'GET',path,headers:{Authorization:'Bearer '+ADMIN,Accept:'application/json'}},r=>{let n=0,a=[];r.on('data',c=>{n+=c.length;if(n>1048576)q.destroy(new Error('bound'));else a.push(c)});r.on('end',()=>{try{resolve({status:r.statusCode,value:JSON.parse(Buffer.concat(a).toString('utf8'))})}catch(e){reject(e)}})});q.setTimeout(8000,()=>q.destroy(new Error('timeout')));q.on('error',reject);q.end()})}
(async()=>{let r=await request('/api/keys');if(r.status!==200||!exact(r.value,['keys','total','allowKeyReveal'])||!Array.isArray(r.value.keys))throw 0;checks.key_api_schema=true;stage='key_name_absent';if(r.value.keys.some(x=>x&&x.name===NAME))throw 0;checks.key_name_absent=true;
stage='combo_api_schema';r=await request('/api/combos');if(r.status!==200||!exact(r.value,['combos','total'])||!Array.isArray(r.value.combos))throw 0;checks.combo_api_schema=true;stage='combo_sentinel_absent';if(r.value.combos.some(x=>x&&(x.name===SENTINEL||x.name==='combo/'+SENTINEL)))throw 0;checks.combo_sentinel_absent=true;
stage='codex_schema';r=await request('/api/providers/'+CODEX);if(r.status!==200||!exact(r.value,['connection'])||!r.value.connection||typeof r.value.connection!=='object')throw 0;checks.codex_schema=true;stage='codex_inactive';if(r.value.connection.id!==CODEX||r.value.connection.provider!=='codex'||r.value.connection.isActive!==false||'accessToken'in r.value.connection||'refreshToken'in r.value.connection||'idToken'in r.value.connection)throw 0;checks.codex_inactive=true;
process.stdout.write(JSON.stringify({status:'KEY_NATIVE_R5_PREFLIGHT_PASS',stage:'complete',checks}));})().catch(()=>{process.stdout.write(JSON.stringify({status:'KEY_NATIVE_R5_PREFLIGHT_STOP',stage,checks}));process.exitCode=1});
'''
...
__BOUNDED_CAPTURE__
LIVE_ID=__LIVE_ID__;LIVE_IMAGE=__LIVE_IMAGE__;LIVE_VOLUME=__LIVE_VOLUME__;CANDIDATE=__CANDIDATE__;CANDIDATE_ID=__CANDIDATE_ID__;VOLUME=__VOLUME__;IMAGE=__IMAGE__;TAG=__TAG__;SOURCE=__SOURCE__;KEY_NAME=__KEY_NAME__;STORE=pathlib.Path(__STORE__);NODE=__NODE__;PREFLIGHT_NODE=__PREFLIGHT_NODE__;ROLLBACK=__ROLLBACK__;MODE=__MODE__
CHECK_NAMES=('candidate_identity','live_identity','image_identity','volume_identity','cloud_false','migration_170','migration_171','schema_columns','credential_shape','server_env_exact','store_absent','key_api_schema','key_name_absent','combo_api_schema','combo_sentinel_absent','codex_schema','codex_inactive')
checks={name:False for name in CHECK_NAMES};stage='candidate_identity';key_id=None;created=False;stored=False;store_identity=None;store_parent_fd=None;rollback_attempted=False;key_inactive_readback=None;failure_category='validation';http_status=None
...
def schema_ready(path):
 ...
 try:
  ...
  migration_170=('170','agent_route_runs') in migrations;migration_171=('171','agent_route_deferred_metrics') in migrations
  if not migration_170 or not migration_171:raise Stop()
  for table,columns in expected.items():
   actual={row[1] for row in db.execute('PRAGMA table_info('+table+')')}
   if actual!=columns:raise Stop()
  db.execute('ROLLBACK')
  return migration_170,migration_171,True
 finally:db.close()
...
try:
 ...
 checks['candidate_identity']=True;stage='live_identity'
 ...
 checks['live_identity']=True;stage='image_identity'
 image=formatted('image','{"id":{{json .Id}},"tags":{{json .RepoTags}}}',TAG)
 if image!={'id':IMAGE,'tags':[TAG]}:raise Stop()
 checks['image_identity']=True;stage='volume_identity'
 ...
 checks['volume_identity']=True;stage='cloud_false';checks['cloud_false']=cloud_false(db_path)
 stage='migration_170';m170,m171,columns=schema_ready(db_path);checks['migration_170']=m170;checks['migration_171']=m171;checks['schema_columns']=columns
 stage='credential_shape';checks['credential_shape']=credential_shape(db_path)
 stage='server_env_exact'
 ...
 del env;checks['server_env_exact']=True
 stage='store_preflight';parent=STORE.parent
 if STORE.name!='auto-switch-candidate-qualification-r5-20260913.key' or parent!=pathlib.Path('/root/.omniroute-qualification') or os.path.lexists(STORE):raise Stop()
 ...
 if MODE=='preflight':
  ...
  if not isinstance(native,dict) or set(native)!={'status','stage','checks'} or native.get('status')!='KEY_NATIVE_R5_PREFLIGHT_PASS' or native.get('stage')!='complete' or not isinstance(native.get('checks'),dict):raise Stop()
  for name in ('key_api_schema','key_name_absent','combo_api_schema','combo_sentinel_absent','codex_schema','codex_inactive'):
   if native['checks'].get(name) is not True:raise Stop()
   checks[name]=True
  print(json.dumps({'status':'CANDIDATE_KEY_R5_PREFLIGHT_PASS','stage':'complete','candidate_id':CANDIDATE_ID,'checks':checks},sort_keys=True,separators=(',',':')));raise SystemExit(0)
 ...
 if rc:
  ...
  if not isinstance(native,dict) or set(native)!=fields or native.get('status')!='KEY_NATIVE_R5_STOP':raise MutationUnknown('remote_envelope')
 ...
 if not isinstance(native,dict) or set(native)!={'status','key_id','secret','codex_inactive_readback'} or native.get('status')!='KEY_NATIVE_R5_PASS' or native.get('codex_inactive_readback') is not True:raise MutationUnknown('remote_envelope')
 ...
 print(json.dumps({'status':'CANDIDATE_KEY_R5_STOP','stage':stage if stage in set(CHECK_NAMES)|{'store_preflight','native_dispatch','native_preflight','provider_preflight','create','patch','readback','db_readback','store','result'} else 'candidate_identity','candidate_id':CANDIDATE_ID,'candidate_running':True if checks['candidate_identity'] else None,'key_id':key_id,'created':created,'stored':stored,'store_removed':removed,'failure_category':failure_category,'http_status':http_status,'rollback_attempted':rollback_attempted,'key_inactive_readback':key_inactive_readback},sort_keys=True,separators=(',',':')));raise SystemExit(1)
finally:
...
def fixture_result():
    return {"status": "CANDIDATE_KEY_R5_PASS", "candidate_id": FIXTURE_CANDIDATE_ID,
...
def validate_result(value, returncode, expected_candidate_id=None):
    ...
    if returncode == 0:
        if not isinstance(value, dict) or set(value) != PASS_FIELDS or value["status"] != "CANDIDATE_KEY_R5_PASS":
    ...
    if isinstance(value, dict) and value.get("status") == "CANDIDATE_KEY_R5_UNKNOWN":
        if (returncode == 0 or set(value) != stop_fields or value["stage"] != "native_dispatch"
            ...
            raise Stop("remote_validation")
        return value
    if not isinstance(value, dict) or set(value) != stop_fields or value["status"] != "CANDIDATE_KEY_R5_STOP":
        raise Stop("remote_validation")
    stages=set(PREFLIGHT_CHECKS)|{"store_preflight", "native_dispatch", "native_preflight", "provider_preflight", "create", "patch", "readback", "db_readback", "store", "result"}
    if value["stage"] not in stages:
        raise Stop("remote_validation")
    if value["candidate_id"] != candidate_id or value["candidate_running"] is not (None if value["stage"] == "candidate_identity" else True):
        raise Stop("remote_validation")
...
def render_preflight_node(admin):
    value = PREFLIGHT_NODE_TEMPLATE
    replacements = {"__ADMIN_B64__": base64.b64encode(admin.encode()).decode(), "__NAME__": json.dumps(KEY_NAME),
                    "__CODEX__": json.dumps(CODEX_CONNECTION), "__SENTINEL__": json.dumps(COMBO_SENTINEL)}
    for marker, replacement in replacements.items():
        if value.count(marker) != 1: raise Stop("local_validation")
        value = value.replace(marker, replacement)
    return value
...
def render_remote(admin, candidate_id=None, mode="execute"):
    candidate_id = candidate_id or CANDIDATE_ID
    if not re.fullmatch(r"[0-9a-f]{64}", candidate_id) or mode not in {"execute", "preflight"}: raise Stop("pending_candidate_id")
    transport = load_transport()
    node = render_node(admin)
    preflight_node = render_preflight_node(admin)
    ...
    values = {"__BOUNDED_CAPTURE__": inspect.getsource(transport.bounded_capture), "__LIVE_ID__": repr(LIVE_ID),
              ...
              "__NODE__": repr(node), "__PREFLIGHT_NODE__": repr(preflight_node), "__ROLLBACK__": repr(rollback),
              "__MODE__": repr(mode)}
...
def load_transport():
    ...
    spec = importlib.util.spec_from_file_location("candidate_key_r5_transport", TRANSPORT)
...
def validate_preflight(value, returncode):
    fields = {"status", "stage", "candidate_id", "checks"}
    if not isinstance(value, dict) or set(value) != fields or value.get("candidate_id") != CANDIDATE_ID:
        raise Stop("remote_validation")
    checks = value.get("checks")
    if not isinstance(checks, dict) or tuple(checks) != tuple(sorted(PREFLIGHT_CHECKS)) or any(type(v) is not bool for v in checks.values()):
        raise Stop("remote_validation")
    if returncode == 0:
        if value["status"] != "CANDIDATE_KEY_R5_PREFLIGHT_PASS" or value["stage"] != "complete" or not all(checks.values()):
            raise Stop("remote_validation")
    elif value["status"] != "CANDIDATE_KEY_R5_PREFLIGHT_STOP" or value["stage"] not in set(PREFLIGHT_CHECKS) | {"store_preflight", "native_preflight"}:
        raise Stop("remote_validation")
    return value
...
def fixed_dependencies(include_preflight=False):
    rows = [(BINDINGS, BINDINGS_SHA256), (TRANSPORT, TRANSPORT_SHA256),
    ...
    if include_preflight:
        rows.append((PREFLIGHT_RESULT, PREFLIGHT_RESULT_SHA256))
    return tuple(rows)


def offline_check():
    if EXECUTION_READY or require_bound() != ["preflight_result"]: raise Stop("local_validation")
    ...
    node=render_node("oma_fixture_secret");payload=render_remote("oma_fixture_secret",FIXTURE_CANDIDATE_ID);preflight_payload=render_remote("oma_fixture_secret",FIXTURE_CANDIDATE_ID,"preflight")
    if b"oma_fixture_secret" in payload or b"bounded_capture" not in payload or b"MODE='preflight'" not in preflight_payload:raise Stop("local_validation")
    ...
    validate_preflight({"status":"CANDIDATE_KEY_R5_PREFLIGHT_PASS","stage":"complete","candidate_id":CANDIDATE_ID,"checks":{name:True for name in sorted(PREFLIGHT_CHECKS)}},0)
    print("CANDIDATE_KEY_R5_PREPARATION_PASS")
...
def review_payload():
    ...
    return {"schema":"auto-switch-candidate-key-r5-preparation-review/v1","launcher_sha256":digest(pathlib.Path(__file__)),
            ...
            "execution_ready":False,"pending":["preflight_result"]}
...
def run_preflight():
    if not all(regular(path) and digest(path)==expected for path,expected in fixed_dependencies()):raise Stop("local_validation")
    validate_reconciliation(parse_receipt(RECONCILIATION_RESULT),CANDIDATE_ID)
    if "PROGRAMDATA" not in {key.upper() for key in os.environ} or os.environ.get("PYTHONOPTIMIZE"):
        raise Stop("local_validation")
    ...
    payload=render_remote(admin,mode="preflight");transport=load_transport();command=[transport.SSH,"-T",*transport.options("192.168.1.68"),"belladmin@192.168.1.68","sudo -n python3 -"]
    saved=os.environ.pop("OMNIROUTE_ADMIN_TOKEN",None)
    try:rc,out,err=transport.bounded_capture(command,payload,180)
    finally:
        if saved is not None:os.environ["OMNIROUTE_ADMIN_TOKEN"]=saved
    if err:raise Stop("remote_validation")
    value=validate_preflight(json.loads(out),rc);print(json.dumps({"schema":PREFLIGHT_SCHEMA,"remote":value},sort_keys=True,separators=(",",":")))
    return 0 if rc==0 else 1


def execute(reviewed):
    if not EXECUTION_READY or require_bound():raise Stop("pending_bindings")
    if not all(regular(path) and digest(path)==expected for path,expected in fixed_dependencies(True)):raise Stop("local_validation")
    preflight_receipt=parse_receipt(PREFLIGHT_RESULT)
    if set(preflight_receipt)!={"schema","remote"} or preflight_receipt.get("schema")!=PREFLIGHT_SCHEMA:raise Stop("local_validation")
    validate_preflight(preflight_receipt["remote"],0)
    ...
    try:
        ...
        status="PASS" if rc==0 else ("UNKNOWN" if value["status"]=="CANDIDATE_KEY_R5_UNKNOWN" else "STOP")
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument("--self-check",action="store_true");parser.add_argument("--render-review-payload",action="store_true");parser.add_argument("--preflight",action="store_true");parser.add_argument("--execute-reviewed",type=pathlib.Path);args=parser.parse_args()
    if sum((args.self_check,args.render_review_payload,args.preflight,bool(args.execute_reviewed)))!=1:raise Stop("local_validation")
    ...
    if args.preflight:return run_preflight()
...
if __name__=="__main__":
    try:raise SystemExit(main())
        ...
        print(json.dumps({"status":"CANDIDATE_KEY_R5_NOT_EXECUTABLE","stage":stage,"execution_ready":False},sort_keys=True));raise SystemExit(1)
