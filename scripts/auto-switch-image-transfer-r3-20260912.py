...
def execute(reviewed):
    ...
    print(json.dumps({"status":"IMAGE_TRANSFER_R3_PASS","size":info["size"],"sha256":info["sha256"],**result},sort_keys=True))
...
if __name__ == "__main__":
    try: main()
        ...
        print(json.dumps({"status":"IMAGE_TRANSFER_R3_STOP","stage":stage,"reason":type(error).__name__,"detail":detail,"hint":"TIMEOUT" if isinstance(error,Stop) and "timeout" in str(error) else "STAGE_FAILED"}))
        sys.exit(1)
