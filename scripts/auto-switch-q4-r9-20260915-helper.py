...
def _validate(request, runtime):
    ...
    if pathlib.Path(__file__).read_bytes() != bundle["helper"]: raise ValueError("executing helper drift")
    ...
    if action.get("leaf_root") != "." or action.get("secret_path") != "key": raise ValueError("unbound root")
...
def execute(request, runtime, secret_path):
    ...
    secret_path = pathlib.Path(secret_path).resolve()
    root = secret_path.parent.resolve()
    if secret_path.name != action["secret_path"] or not secret_path.is_file(): raise ValueError("secret drift")
    ...
    try:
        ...
        passed = passed and completion["status"] == 200 and completion["candidate_id"] == action["runtime"]["candidate_id"] and completion["model"] == action["runtime"]["model"] and completion["connection_id"] == action["runtime"]["connection_id"] and type(completion["request_count"]) is int and completion["request_count"] == 1 and isinstance(completion["request_id"], str) and bool(completion["request_id"]) and type(completion["content_bytes"]) is int and completion["content_bytes"] > 0
    except Exception:
        return 1
    finally:
