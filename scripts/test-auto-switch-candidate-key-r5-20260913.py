"""Focused offline boundaries for the unbound R5 native qualification-key source."""
import copy
import ast
...
LAUNCHER = ROOT / "scripts/auto-switch-candidate-key-r5-20260913.py"

assert LAUNCHER.is_file(), "R5 qualification-key launcher missing"
spec = importlib.util.spec_from_file_location("candidate_key_r5", LAUNCHER)
...
assert module.require_bound() == ["preflight_result"]
...
assert native_result == {"create_status": 201, "patch_status": 200, "get_status": 200,
                         ...
                         "migration_rows": [{"version": "170", "name": "agent_route_runs"},
                                            {"version": "171", "name": "agent_route_deferred_metrics"}]}

rendered = module.render_remote("oma_fixture_secret", module.CANDIDATE_ID)
preflight_node = module.render_preflight_node("oma_fixture_secret")
...
assert "method:'GET'" in preflight_node and "method:'POST'" not in preflight_node and "method:'PATCH'" not in preflight_node


def run_rendered_remote(outcome_mode, remote_mode="execute"):
    with tempfile.TemporaryDirectory() as temporary:
        ...
        db.executescript("""
          ...
          INSERT INTO _omniroute_migrations VALUES('170','agent_route_runs');
          INSERT INTO _omniroute_migrations VALUES('171','agent_route_deferred_metrics');
 ...
 mode={outcome_mode!r};remote_mode={remote_mode!r};database={str(database)!r};candidate={candidate!r};live={live!r}
 if args[:3]==['docker','container','inspect']:
  ...
  if remote_mode=='preflight':
   value={{'status':'KEY_NATIVE_R5_PREFLIGHT_PASS','stage':'complete','checks':{{'key_api_schema':True,'key_name_absent':True,'combo_api_schema':True,'combo_sentinel_absent':True,'codex_schema':True,'codex_inactive':True}}}}
  ...
  value={{'status':'KEY_NATIVE_R5_PASS','key_id':{key_id!r},'secret':'sk-1111111111111111-111111-11111111','codex_inactive_readback':True}}
 else:raise RuntimeError('FIXTURE_UNEXPECTED')
 return 0,json.dumps(value,separators=(',',':')).encode(),b''
'''
        source = module.render_remote("oma_fixture_secret", module.CANDIDATE_ID, remote_mode).decode("utf-8")
...
assert passed_code == 0 and passed_remote["status"] == "CANDIDATE_KEY_R5_PASS" and store_exists and committed == 1
module.validate_result(passed_remote, passed_code)
preflight_code, preflight_remote, preflight_store, committed = run_rendered_remote("pass", "preflight")
assert preflight_code == 0 and preflight_remote["status"] == "CANDIDATE_KEY_R5_PREFLIGHT_PASS"
assert not preflight_store and committed == 0 and all(preflight_remote["checks"].values())
module.validate_preflight(preflight_remote, preflight_code)
bad_preflight = copy.deepcopy(preflight_remote)
bad_preflight["checks"]["migration_170"] = False
try:
    module.validate_preflight(bad_preflight, 0)
except module.Stop:
    pass
else:
    raise AssertionError("preflight accepted a failed native migration predicate")
for mode, category in (("timeout", "child_timeout"), ("overflow", "child_output"), ("malformed", "remote_envelope")):
    ...
    assert unknown_code == 2 and unknown_remote["status"] == "CANDIDATE_KEY_R5_UNKNOWN" and not unknown_store and committed == 1
...
def rejected(mutator):
    ...
    try:
        ...
        return
    raise AssertionError("invalid R5 key result accepted")
...
unknown = {"status": "CANDIDATE_KEY_R5_UNKNOWN", "stage": "native_dispatch",
...
print("PASS: R5 key identity, native migration metadata, policy, preflight and terminal receipt")
